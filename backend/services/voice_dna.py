...
def build_voice_imprint(
    ...
    """
    Extract safe pattern examples from the creator's fingerprint.
    Runtime should learn cadence and structure, not paste transcript lines.
    """
    sfp = sanitize_style_fingerprint_for_runtime(_coerce_dict(creator_profile.get("style_fingerprint")))
    ...
    for text, ex_mode in all_examples:
        ...
        if len(text) < 15:
            continue
        if not clean_style_phrase_list([text], limit=1):
            continue
        key = text[:80].lower()
        if key in seen:
            continue
    ...
    # Build annotated imprint without framing these as reusable lines.
    lines = [
        "VOICE PATTERN IMPRINT (behavioral pattern references only; do not copy wording):"
    ]
...
def build_response_scaffold(
    ...
    openings = clean_style_phrase_list(mechanics.get("signature_openings") or [], limit=5)
    ...
    landings = clean_style_phrase_list(mechanics.get("signature_landings") or [], limit=5)
...
def score_voice_fidelity(
    ...
    if word_count >= 30 and sig_phrases:
        ...
        if sig_hits == 0:
            score -= 0.05
            gaps.append("no_voice_pattern_phrases")
...
_QUOTEY_CONTENT_HOOK_RE = re.compile(
    r"\b("
    ...
    for chunk in rag_chunks:
        ...
        for s in sentences:
            ...
            if any(lower.startswith(skip) for skip in _ECHO_SKIP_STARTS):
                continue
            # Skip transcript hooks/catchphrases. They are useful style signal
            # during fingerprinting, but pasted into a DM they read as quoted
            # content instead of a natural answer.
            if _QUOTEY_CONTENT_HOOK_RE.search(s):
                continue
...
def build_voice_echo_block(
    ...
    lines = [
        ...
        "absorb the vocabulary, slang, and energy without pasting full hooks):"
    ]
    for echo in echoes:
        truncated = echo if len(echo) <= 200 else echo[:197] + "..."
        lines.append(f'  - "{truncated}"')
    lines.append(
        "Borrow the feel and word choice, not whole catchphrases. Use at most "
        "one short creator-flavored phrase in the answer, and skip it if the "
        "conversation already used one recently."
    )
    return "\n".join(lines)
...
def build_voice_dna_block(
    ...
    if conversation_tracker:
        sfp = sanitize_style_fingerprint_for_runtime(_coerce_dict(creator_profile.get("style_fingerprint")))
        lexical = _coerce_dict(sfp.get("lexical_rules"))
        sig = clean_style_phrase_list(lexical.get("signature_phrases") or [], limit=10)
