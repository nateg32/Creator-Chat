...
def persist_search_items(
    ...
    if _table_column_exists("scrape_runs", "creator_id"):
        db.execute_update(
            """
            INSERT INTO scrape_runs (id, creator_id, source_url, platform, mode, creator_handle, items_found)
            ...
            """,
            (search_run_id, creator_id, source_url, platform, mode, creator_handle, len(normalized_items)),
        )
    else:
        db.execute_update(
            """
    ...
    insert_query = """
        ...
        ON CONFLICT (source_url) DO UPDATE SET
            ...
            transcript = CASE
                WHEN COALESCE(EXCLUDED.transcript, '') <> '' THEN EXCLUDED.transcript
                ELSE scrape_items.transcript
            END,
            transcript_status = CASE
                WHEN COALESCE(EXCLUDED.transcript, '') <> '' THEN EXCLUDED.transcript_status
                WHEN scrape_items.transcript_status = 'present' THEN scrape_items.transcript_status
            ...
            review_status = CASE
                WHEN scrape_items.review_status IN ('approved', 'denied')
                 AND COALESCE(scrape_items.content_fingerprint, 0) = COALESCE(EXCLUDED.content_fingerprint, 0)
    ...
    for item in normalized_items:
        try:
            ...
            title_for_index = item.get('title') or meta.get('title') or item.get('caption') or ''
            ...
            norm_text = compute_normalized_text(title_for_index, item.get('description', ''), item.get('caption', ''))
            ...
            if cached_asset:
                item_transcript = cached_asset.get('transcript') or ''
                meta = apply_transcript_asset_to_metadata(meta, cached_asset)
                meta['transcript_status'] = cached_asset.get('transcript_status') or 'present'
                metadata_json = json.dumps(meta, default=str)
            norm_status = resolve_transcript_status(item_transcript, item.get('transcript_status', 'missing'))
            db_item_id = db.execute_insert(
                insert_query,
                (
                    str(uuid.uuid4()), search_run_id, item_creator_handle, item['content_type'],
                    item['source_url'], item.get('caption'), item_transcript,
