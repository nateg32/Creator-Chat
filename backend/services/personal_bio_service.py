...
def _user_explicitly_requested_source(question: str) -> bool:
    return bool(_SOURCE_REQUEST_RE.search(str(question or "")))


def _should_suppress_simple_identity_sources(question: str, evidence_plan: Optional[Any]) -> bool:
    if _user_explicitly_requested_source(question):
        return False
    if str(getattr(evidence_plan, "query_goal", "") or "").lower() != "identity_lookup":
        return False
    policy = classify_creator_fact_query(question)
    return policy.kind == "public_profile" and policy.focus in {
        "relationship_status",
        "family",
        "age",
        "public_location",
...
_INTENT_SLICE_PATTERNS = (
    re.compile(r"\bwhy\b", re.IGNORECASE),
    re.compile(r"\bhow\s+(?:can|do|should|would|did)\b", re.IGNORECASE),
    re.compile(r"\bwhere\s+can\b", re.IGNORECASE),
    re.compile(r"\bwhen\b|\bwhat\s+(?:year|date|month)\b", re.IGNORECASE),
    re.compile(r"\bwhat\s+(?:was|is|are|do|did)\b", re.IGNORECASE),
    re.compile(r"\bwho\b", re.IGNORECASE),
    re.compile(r"\b(?:watch|video|episode|podcast|resource|link|source)\b", re.IGNORECASE),
)
_COMPOUND_CONNECTOR_RE = re.compile(r"\b(?:also|plus|as well|and|then)\b|[,;]", re.IGNORECASE)
_REASON_OR_CREATION_RE = re.compile(
    r"\b(?:why|what\s+(?:inspired|made|led|motivated))\b.{0,80}\b(?:write|create|make|build|publish|release|launch)\b",
    re.IGNORECASE,
)
_APPLICATION_RE = re.compile(
    r"\b(?:how\s+(?:can|do|should|would)\s+(?:i|we)|apply|use|implement|put\s+.+\s+into\s+practice)\b",
    re.IGNORECASE,
)
_BROAD_JOURNEY_RE = re.compile(
    r"\b(?:journey|story|background|get\s+(?:rich|wealthy|successful)|become\s+(?:rich|wealthy|successful)|make\s+(?:your|ur|his|her|their)?\s*(?:money|wealth|fortune))\b",
    re.IGNORECASE,
)
_SOURCE_METADATA_RE = re.compile(
    r"\b(?:channel|length|views?|subscribers?|duration|published\s+time|upload\s+date|source\s+title|title)\s*:",
    re.IGNORECASE,
)


def _has_multiple_user_intents(question: str) -> bool:
    text = re.sub(r"\s+", " ", str(question or "").strip())
    if not text or not _COMPOUND_CONNECTOR_RE.search(text):
        return False
    hits = sum(1 for pattern in _INTENT_SLICE_PATTERNS if pattern.search(text))
    repeated_question_words = len(re.findall(r"\b(?:why|how|where|when|what|who)\b", text, flags=re.IGNORECASE))
    return hits >= 2 or repeated_question_words >= 2


def _should_use_reasoning_synthesis(question: str, policy: Any, evidence_plan: Optional[Any]) -> bool:
    """Prefer the model brain when the user is asking for meaning, application, or several things at once."""
    text = re.sub(r"\s+", " ", str(question or "").strip())
    if not text:
        return False
    goal = str(getattr(evidence_plan, "query_goal", "") or "").lower()
    policy_kind = str(getattr(policy, "kind", "") or "").lower()
    if _has_multiple_user_intents(text):
        return True
    if _REASON_OR_CREATION_RE.search(text):
        return True
    if is_creator_journey_turning_point_question(text):
        return True
    if _APPLICATION_RE.search(text) and goal in {"availability_lookup", "resource_lookup", "entity_overview", "entity_confirmation"}:
        return True
    if policy_kind == "creator_journey":
        focus = _extract_timeline_focus(text)
        if not focus or _BROAD_JOURNEY_RE.search(text):
            return True
    return False


def _looks_like_source_metadata_text(text: str) -> bool:
    candidate = str(text or "")
    if not candidate:
        return False
    if _SOURCE_METADATA_RE.search(candidate):
        return True
    return bool(re.search(r"\b(?:length|views?)\s*:\s*(?:\d|$)", candidate, re.IGNORECASE))
...
def _extract_fact_value_from_text(fact_field: str, text: str) -> str:
    ...
    if lowered_field in {"revenue", "income", "earnings", "valuation", "net_worth"}:
        money_match = re.search(
            r"(?:over\s+|more than\s+|around\s+|about\s+|approximately\s+)?(?:\$\s?\d[\d,]*(?:\.\d+)?\s?(?:[kKmMbB]|\bmillion\b|\bbillion\b)?\+?|\b\d[\d,]*(?:\.\d+)?\s?(?:[kKmMbB]|\bmillion\b|\bbillion\b)\+?)",
            candidate_text,
            re.IGNORECASE,
        )
        if money_match:
            return re.sub(r"\s+", " ", money_match.group(0)).strip()

    if lowered_field in {"followers", "subscribers", "students", "members", "latest_episode"}:
...
def _extract_journey_reason(blob: str, creator_name: str = "") -> str:
    ...
    for segment in segments:
        ...
        if not cleaned_segment:
            continue
        if _looks_like_source_metadata_text(cleaned_segment):
            continue
        for pattern in patterns:
            ...
            if _looks_like_source_metadata_text(normalized_candidate):
                continue
...
class PersonalBioService:
    ...
    def _creator_focus_label(self, creator_profile: Optional[Dict[str, Any]]) -> str:
        style_fp = self._coerce_dict((creator_profile or {}).get("style_fingerprint"))
        domain_map = self._coerce_dict(style_fp.get("domain_map"))
        creator_lane = str(domain_map.get("creator_lane") or "").strip()
        if creator_lane:
            return creator_lane

        category = str((creator_profile or {}).get("creator_category") or "").lower()
        if any(token in category for token in ("fitness", "health", "gym", "nutrition", "bodybuilding")):
            return "training, nutrition, and the work you are putting in"
        if any(token in category for token in ("trading", "forex", "crypto", "invest")):
            return "trading, execution, and the process"
        if any(token in category for token in ("business", "entrepreneur", "startup", "sales", "marketing")):
            return "business, leverage, and what you are building"
        if any(token in category for token in ("creator", "content", "youtube", "media")):
            return "content, audience, and the work you are creating"
        return "the public work and ideas I share"
    ...
    def _boundary_tone_hint(self, creator_profile: Optional[Dict[str, Any]], voice_profile: Optional[Dict[str, Any]]) -> str:
        style_fp = self._coerce_dict((creator_profile or {}).get("style_fingerprint"))
        voice = self._coerce_dict(voice_profile)
        identity_signature = self._coerce_dict(style_fp.get("identity_signature"))
        mode_matrix = self._coerce_dict(style_fp.get("mode_matrix"))
        boundary = self._coerce_dict(mode_matrix.get("boundary"))
        pressure_map = self._coerce_dict(style_fp.get("pressure_map"))
        unknown_policy = self._coerce_dict(style_fp.get("unknown_topic_policy"))
        pieces = [
            identity_signature.get("private_boundary_style"),
            boundary.get("private_life_style"),
            pressure_map.get("asked_private_question"),
            unknown_policy.get("boundary_style"),
            voice.get("private_boundary_style"),
            voice.get("boundary_style"),
        ]
        return " ".join(str(piece or "") for piece in pieces).lower()

    def _persona_boundary_opening(
        self,
        ...
        category = str((creator_profile or {}).get("creator_category") or "").lower()
        hint = self._boundary_tone_hint(creator_profile, voice_profile)

        if question_type == "private_sensitive":
            if any(token in hint for token in ("playful", "joke", "humor", "banter")):
                return "That one stays off the table."
            return "That is not the lane I get into."

        if any(token in hint for token in ("blunt", "direct", "hard", "firm", "tough", "pushback", "no bs", "no-bull")):
            return "I hear you, but I keep that side private."
        if any(token in hint for token in ("warm", "soft", "gentle", "calm", "empathetic", "respectful")):
            return "I appreciate you asking, but I keep that side private."

        if any(token in category for token in ("fitness", "health", "gym", "nutrition", "bodybuilding")):
            return "I keep that side private."
        if any(token in category for token in ("trading", "forex", "crypto", "invest")):
            return "I keep that off the public chart."
        if any(token in category for token in ("business", "entrepreneur", "startup", "sales", "marketing")):
            return "I do not make that part of the public operating system."
        if any(token in category for token in ("creator", "content", "youtube", "media")):
            return "I keep that part off-camera."

        return "I appreciate you asking, but I keep that side private."

    def _private_boundary_answer(
        self,
        creator_profile: Optional[Dict[str, Any]],
        voice_profile: Optional[Dict[str, Any]],
        topic: str,
        ...
        focus = self._creator_focus_label(creator_profile)
        opening = self._persona_boundary_opening(creator_profile, voice_profile, topic, question_type)
        if question_type == "private_sensitive":
            return f"{opening} I keep private stuff private, and I would rather keep this focused on {focus}."
        if topic == "politics_religion":
            return f"{opening} I am here to focus on {focus} with you."
        if topic in {"relationship", "family", "location"}:
            return f"{opening} I am here to focus on {focus} with you."
        return f"I have not shared that publicly, so I would not want to guess. I am here to focus on {focus} with you."

    def _should_decline_private_without_search(
        self,
        ...
        *,
        ...
        fact_policy: Optional[Any] = None,
    ) -> bool:
        if question_type == "private_sensitive":
            return True
        public_fact_goal = str(getattr(evidence_plan, "query_goal", "") or "").lower() in {
            "timeline_lookup",
            "price_lookup",
            "stat_lookup",
            "current_stat_lookup",
        ...
        public_fact_world = str(getattr(evidence_plan, "primary_world", "") or "").lower() in {"creator_world", "live_world"} and bool(
            getattr(evidence_plan, "should_verify", False) or getattr(evidence_plan, "should_search_web", False)
        )
        policy_requires_public_evidence = bool(
            fact_policy
            and getattr(fact_policy, "requires_web", False)
            and getattr(fact_policy, "requires_verified_sources", False)
        )
        if public_fact_goal or public_fact_world or policy_requires_public_evidence:
            return False
        if question_type != "personal_bio":
            return False
        policy = decision_service.get_policy({"decision_policy": decision_policy})
        boundary = (policy.get("privacy_boundary") or {}).get(topic)
        return boundary in {"private", "avoid"}
    ...
    def handle_personal_question(
        ...
        # (for "ingested only"), this service must stay inside ingested corpus
        # evidence without silently re-enabling search or public fact caches.
        effective_allow_web = bool(allow_web and researcher_enabled)
        strict_internal_only = not effective_allow_web
        ...
        public_fact_query = (
            ...
            or evidence_plan.query_goal == "identity_lookup"
            or policy.requires_web
            or (
        ...
        if strict_internal_only:
            public_fact_query = False
        use_reasoning_synthesis = bool(
            public_fact_query
            and _should_use_reasoning_synthesis(resolved_question, policy, evidence_plan)
        )
        if self._should_decline_private_without_search(
            decision_policy,
            q_type,
            topic,
            evidence_plan=evidence_plan,
            ...
            return {
                "answer": self._private_boundary_answer(creator_profile, voice_profile, topic, q_type),
                "confidence": "LOW",
                "sources": [],
                "move": "DECLINE_PRIVATE",
        ...
        journey_turning_point = bool(
            policy.kind == "creator_journey"
            and is_creator_journey_turning_point_question(resolved_question)
        )
        if journey_turning_point:
            fact_field = "journey_turning_point"
            use_reasoning_synthesis = True
        ...
        if (
            evidence_plan.query_goal == "availability_lookup"
            and not use_reasoning_synthesis
            and resolved_entity
            and (resolved_entity.get("official_urls") or [])
        ):
            return {
        ...
        internal_facts = (
            ...
            if evidence_plan.should_search_corpus or use_reasoning_synthesis
            else []
        )
        ...
        user_visible_sources = [] if _should_suppress_simple_identity_sources(resolved_question, evidence_plan) else all_evidence
        ...
        if public_fact_query:
            if extracted_fact and not use_reasoning_synthesis:
            ...
            if requires_verified_timeline and not web_facts and not use_reasoning_synthesis:
                fallback_answer = self._public_fact_fallback(
                    resolved_question,
                    creator_name,
            ...
            if not use_reasoning_synthesis:
    ...
    def _build_public_fact_search_queries(
        ...
        *,
        ...
        if query_goal == "timeline_lookup":
        ...
        elif query_goal == "journey_lookup" and is_creator_journey_turning_point_question(question):
            queries.extend([
                f'{creator_name} turning point life story',
                f'{creator_name} what made him turn his life around',
                f'{creator_name} what made her turn her life around',
                f'{creator_name} rock bottom story',
                f'{creator_name} lowest point changed life',
                f'{creator_name} why changed life',
            ])
            ...
            if policy.kind == "public_profile":
                if policy.fact_field == "age":
                    queries.extend([
                        f'{creator_name} age birthday born',
                        f'{creator_name} date of birth',
                    ])
                elif policy.fact_field == "relationship_status":
                    queries.extend([
                        f'{creator_name} married spouse partner',
                        f'{creator_name} wife husband girlfriend boyfriend',
                    ])
                elif policy.fact_field == "family":
                    queries.extend([
                        f'{creator_name} children kids family',
                        f'{creator_name} son daughter family',
                    ])
                elif policy.fact_field == "public_location":
                    queries.extend([
                        f'{creator_name} hometown born based',
                        f'{creator_name} where from based in',
                    ])
                else:
                    queries.append(f'{creator_name} public biography')
            else:
                queries.extend([
        ...
        if query_goal == "timeline_lookup":
            ...
            if is_creator_journey_turning_point_question(question):
                queries.extend([
                    f'{creator_name} turnaround story',
                    f'{creator_name} turning point interview',
                    f'{creator_name} what changed his life',
                    f'{creator_name} what changed her life',
                ])
            elif focus_hint:
                queries.extend([
                    f'{creator_name} {focus_hint} journey',
            ...
            for token, phrase in (
                ...
                if token in lowered_question:
                    stat_terms.append(phrase)
            stat_phrase = " ".join(stat_terms[:3]) or "public stats"
            if subject_hint:
                queries.extend([
                    f'{creator_name} "{subject_hint}" {stat_phrase}',
                    f'"{subject_hint}" {stat_phrase} current',
                ])
            queries.append(f"{creator_name} {stat_phrase} current public")
        elif query_goal == "role_lookup":
    ...
    def _render_structured_fact_answer(
        ...
        *,
        ...
        if policy.kind == "creator_journey":
            ...
            if not focus or focus in {"it", "this", "that", "one"}:
                return ""
            if reason:
        ...
        if fact_field in {"revenue", "income", "earnings", "valuation", "net_worth"} and value:
            label = fact_field.replace("_", " ")
            if subject and subject.lower() not in {"it", "that", "this"}:
                possessive = f"{subject}'" if subject.endswith("s") else f"{subject}'s"
                return f"Publicly, {possessive} {label} is reported around {value}."
            return f"Publicly, my {label} is reported around {value}."
    ...
    def _extract_structured_fact_candidate(
        ...
        if fact_field in {"followers", "latest_episode", "valuation", "net_worth", "revenue", "income", "earnings"}:
            if fact_field in {"valuation", "net_worth", "revenue", "income", "earnings"}:
            ...
            if count_match:
                value = re.sub(r"\s+", " ", count_match.group(0)).strip()
                label = fact_field.replace("_", " ")
                if fact_field in {"revenue", "income", "earnings", "valuation", "net_worth"} and subject:
                    possessive = f"{subject}'" if subject.endswith("s") else f"{subject}'s"
                    answer_text = f"Publicly, {possessive} {label} is reported around {value}."
                else:
    ...
    def _answer_entity_availability(self, entity: Dict[str, Any]) -> str:
        ...
        if not official_urls:
            return "I do not have a verified current link for that in this chat yet."
    ...
    def _synthesize_public_fact_answer(
        ...
        for e in evidence:
            ...
            evidence_lines.append(f"- [{e.get('source', 'unknown')}]: {title_prefix}{text[:300]}")
        evidence_text = "\n".join(evidence_lines)
        vp_json = json.dumps(voice_profile, indent=2)
        journey_rule = ""
        if policy.kind == "creator_journey":
            journey_rule = """
CREATOR JOURNEY MODE:
- Use Gemini reasoning over the public evidence titles, snippets, and grounded overview text.
- Do NOT require a verbatim quote before answering. If the evidence clearly identifies a source about why the creator started or continued something, synthesize the public story cautiously in first person.
- If the user asks what made, led, motivated, or forced the creator to turn their life around, answer the catalyst or turning point first. Do not repeat a generic biography from an earlier turn.
- If the evidence only proves the source exists but gives no actual reason, say the cleanest public source is about that question and give the safest high-level answer the evidence supports. Never output the "couldn't pin down one clean public quote" fallback.
- If a specific video, episode, article, or post is directly useful for the user's question, you may add one short natural handoff line in the creator's voice. Do not force a resource mention when it does not help.
"""
...
RULES:
1. Answer directly from the evidence in 1-3 sentences, or up to 4 short sentences if the user asks multiple things.
2. Answer every distinct part of the user's question. If they ask "how" and "where", cover both. If they ask "journey" and "how you got rich", synthesize the story rather than only naming a source.
3. If the evidence contains a date, title, platform, availability detail, role, company name, or a clear reason or motivation from your public story, use the concrete point that actually answers the user's intent.
4. For why, motivation, journey, and application questions, do not answer with only a publication date, source title, platform, or metadata.
5. Treat source titles and fields like Channel, Length, Views, URL, and platform labels as metadata, not biography. Never turn them into "I got into..." or "I did this because..." facts.
6. Never say "I haven't talked about that publicly" about your own public work.
7. Never say "I don't have that in front of me" about your own book, product, or release.
8. Never invent facts. If the evidence is still insufficient, direct the user to a concrete official source.
9. Never mention evidence, transcripts, search, verification steps, or that you pulled anything up.
10. If a specific attached resource naturally helps the user go deeper, mention it conversationally once. Never say "I've attached the link below" or generic "link below" copy.
11. Keep it natural and first-person, like a direct answer to the user.
...
"""
    ...
    def _clean_model_fallback_answer(self, raw_answer: Any) -> str:
        answer = str(raw_answer or "").strip().strip('"').strip()
        if not answer:
            return ""
        try:
            parsed = json.loads(answer)
            if isinstance(parsed, dict):
                answer = str(parsed.get("answer") or "").strip().strip('"').strip()
        except Exception:
            pass
        lowered = answer.lower()
        if lowered in {"fallback", "none", "unknown", "n/a"}:
            return ""
        if "classification" in lowered and "answer" not in lowered:
            return ""
        if len(answer.split()) < 4:
            return ""
        return answer

    def _synthesize_public_fact_fallback(
        self,
        question: str,
        creator_name: str,
        voice_profile: Optional[Dict[str, Any]],
        *,
        ...
        official_urls = [url for url in (official_urls or []) if url]
        official_hint = official_urls[0] if official_urls else ""
        vp_json = json.dumps(voice_profile or {}, indent=2)
        category = policy_kind or query_goal or "public fact"
        system_prompt = f"""
You are {creator_name}.

The user asked a public factual question, but the pipeline did not find enough verified evidence to answer it as a fact.

Voice Profile:
{vp_json}

Write the fallback using Gemini reasoning, not a canned support script.

Rules:
1. Stay in the creator's first-person voice and match the user's conversational energy.
2. Do not invent dates, numbers, motivations, pricing, catalog counts, or private details.
3. Do not say "I've attached the link below", "check my official website", "current public listing", or any generic handoff line.
4. If an official URL is supplied and it truly helps, mention it naturally once. Otherwise do not force a source mention.
5. Max 2 short sentences. No markdown. No raw reasoning.
...
Official URL if useful: {official_hint}
"""
        try:
            resp = rag.generate_chat_completion(
                messages=[
            ...
            data = json.loads(resp)
            return self._clean_model_fallback_answer(data.get("answer"))
        except Exception as e:
            logger.debug("Public fact fallback synthesis failed: %s", e)
            return ""
    ...
    def _public_fact_fallback(
        ...
        *,
        ...
        model_answer = self._synthesize_public_fact_fallback(
            question,
            creator_name,
            voice_profile,
            query_goal=query_goal,
        ...
        if _looks_like_catalog_question(lowered, query_goal):
            if query_goal == "entity_catalog_lookup" and official_urls:
                return f"I do not have a verified full list in this chat yet. The cleanest place to start is {official_urls[0]}."
            if "book" in lowered:
                return (
        ...
        if _looks_like_timeline_question(lowered, query_goal):
            if _is_publication_timeline_question(question):
                return (
                    "I do not have a verified publication date in this chat yet, so I am not going to guess the timeline."
                )
            timeline_focus = _extract_timeline_focus(question)
            if timeline_focus:
                return (
                    f"I do not have a verified date for when I started {timeline_focus} in this chat yet, so I am not going to make up the timeline."
                )
            return (
        ...
        if query_goal == "journey_lookup" or policy.kind == "creator_journey":
            ...
            if journey_focus:
                return (
                    f"I do not have enough verified context here to tell the full {journey_focus} story cleanly, so I would rather answer that with a source than riff from memory."
                )
            return (
                "I do not have enough verified context here to tell that story cleanly, so I would rather answer it with a source than riff from memory."
            )
        if query_goal in {"price_lookup"} or any(token in lowered for token in ["price", "pricing"]):
            return (
                "I do not have that exact current public detail verified in this chat yet, so I am not going to make up a number."
            )
        if query_goal in {"availability_lookup", "resource_lookup"}:
            if official_urls:
                return f"I do not have enough verified context to answer that fully here. The cleanest place to start is {official_urls[0]}."
            return (
                "I do not have enough verified context to answer that fully here, so I am not going to invent the listing."
            )
        if query_goal in {"current_stat_lookup", "stat_lookup"} or any(
            token in lowered
            for token in [
                "followers",
                "subscribers",
            ...
            return (
                "I do not have a verified current number in this chat yet, so I am not going to make one up."
            )
        return (
...
Move-Specific Logic:
- DECLINE_PRIVATE: Write a creator-natural boundary and briefly redirect to the creator's public lane.
- UNCERTAINTY: Say the exact detail is not verified cleanly enough to answer as fact, then keep the reply useful or ask for a sharper angle.
- NO DISCLAIMERS.
...
"""
        try:
            ...
            data = json.loads(resp)
            return {
                "answer": data.get("answer") or self._generate_uncertain_response(voice_profile),
