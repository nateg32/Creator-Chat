...
class StrongholdGuardService:
    ...
    def generate_boundary_message(
        ...
        signature_phrases = clean_style_phrase_list(
            lexical.get("signature_phrases") or style_fp.get("signature_phrases") or [],
            limit=4,
        )
        ...
        signature_hint = (
            f"Optional vocabulary pattern cues, not lines to copy: {', '.join(signature_phrases)}."
...
- Acknowledges the topic in one beat without answering its substance. Do not define, explain, solve, teach, rank, or give facts about the off-topic request.
- Then pivots. {pivot_instruction}
- Do not mention earlier user questions unless the pivot instruction names a recent topic.
- If the user asks about spouse/marriage status, public family facts, age, birthplace, hometown, or where the creator is publicly based, answer only when verified context supports it. If the user asks about a private belief, sex/body detail, home address, private contact info, private family/location detail, or any fact that is not publicly available, say that side is private or not something you have put out publicly, then pivot in your voice.
- If the request is harmful, illegal, exploitative, or dangerous, refuse the action in your voice without giving instructions, then redirect to a safe next step or the creator's actual domain.
