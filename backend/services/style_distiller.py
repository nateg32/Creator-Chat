...
class StyleDistiller:
    ...
    def get_style_dna(self, creator_id: int, creator_profile: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        creator_profile = sanitize_style_fingerprint_for_runtime(creator_profile)
        ...
        dna["lexical"]["signature_phrases"] = clean_style_phrase_list(
            lexical_rules.get("signature_phrases") or creator_profile.get("signature_phrases") or [],
            limit=4,
        )
...
KEY VOCABULARY PATTERNS: {json.dumps(dna['lexical']['high_signal_vocab'][:12])}
...
ENSURE: Use the behavioral style signals above, not pasted catchphrases or transcript hooks.
""".strip()
