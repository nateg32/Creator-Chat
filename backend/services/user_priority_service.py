...
class UserPriorityService:
    ...
    def detect_user_state(
        ...
        from backend.services.emotional_intelligence import detect_message_vibe

        local_vibe = detect_message_vibe(question, history or [])
        try:
            creator_row = creator_profile or {}
            state = classifiers.classify_all(question, history or [], creator_row)
            if not isinstance(state, dict):
                state = {}
            state["local_vibe"] = local_vibe
            if not state.get("emotion") or (state.get("emotion") or {}).get("primary") in {None, "", "neutral"}:
                state["emotion"] = {
                    "primary": local_vibe.get("primary", "neutral"),
                    "intensity": local_vibe.get("intensity", "low"),
                }
            return state
            ...
            return {
                ...
                "clarity_level": "clear",
                "confusion_level": "low",
                "emotional_tone": local_vibe.get("primary", "neutral"),
                "emotion": {
                    "primary": local_vibe.get("primary", "neutral"),
                    "intensity": local_vibe.get("intensity", "low"),
                },
