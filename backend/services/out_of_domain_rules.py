...
def recent_bridge_topic(history: Optional[List[Dict[str, str]]], current_question: str = "") -> Optional[str]:
    ...
    for message in history:
        ...
        if current and content.lower().strip() == current:
            continue
        if _is_bad_bridge_topic(content):
            continue
...
_PRIVATE_OR_SENSITIVE_BRIDGE_RE = re.compile(
    r"\b("
    r"god|religion|religious|faith|pray|prayer|church|politics|political|vote|voted|"
    r"wife|wifey|husband|girlfriend|boyfriend|dating|married|relationship|partner|spouse|missus|misus|mrs|missis|"
    r"dick|penis|cock|boobs|nudes|naked|sex|sexual|body count|virgin|virginity"
    r")\b",
    re.IGNORECASE,
)
_GENERIC_QUESTION_BRIDGE_RE = re.compile(
    r"^\s*(?:what(?:'s|s| is| are)?|who(?:'s| is)?|when|where|why|how|do you|did you|can you|could you)\b",
    re.IGNORECASE,
)


def _is_bad_bridge_topic(content: str) -> bool:
    text = (content or "").strip()
    if not text:
        return True
    if _PRIVATE_OR_SENSITIVE_BRIDGE_RE.search(text):
        return True
    # Bridge topics should be user context or goals, not a previous standalone
    # question. Otherwise one rejected/private question can leak into the next turn.
    if "?" in text and _GENERIC_QUESTION_BRIDGE_RE.search(text):
        return True
    return False
...
# Detects generic answer-engine questions (git tutorials, cooking,
...
_GENERAL_TOPIC_PATTERNS: Dict[str, List[str]] = {
    ...
    "coding": [
        r"\b(what\s+(?:are|is)|what'?s|explain|define|tell\s+me\s+about)\s+(the\s+)?(fundamentals|basics|principles|concepts)\s+of\s+(software\s+engineering|computer\s+science|programming|coding|web\s+development)\b",
        r"\b(software\s+engineering|computer\s+science|programming|coding|web\s+development)\s+(fundamentals|basics|principles|concepts)\b",
        r"\b(what\s+(?:is|are)|what'?s|explain|define)\s+(a\s+)?(breadth\s+first\s+search|depth\s+first\s+search|binary\s+search|linked\s+list|hash\s+map|api|database|server|frontend|backend|software\s+architecture)\b",
...
def detect_general_knowledge_topic(question: str) -> Optional[str]:
    ...
    that any generic answer engine could answer but is unrelated to most creator specialties.
...
def should_redirect_general_knowledge(
    ...
    if topic not in _GENERAL_TOPIC_DOMAIN_MAP:
    ...
    if allowed_for_topic and all_creator_domains.intersection(allowed_for_topic):
        return False
