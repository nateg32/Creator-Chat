...
def _derive_creator_category_from_fingerprint(style_fingerprint: Dict[str, Any]) -> str:
    style_fingerprint = style_fingerprint or {}
    search_profile = style_fingerprint.get("search_profile") or {}
    domain_map = style_fingerprint.get("domain_map") or {}
    candidates = [
        search_profile.get("primary_category") if isinstance(search_profile, dict) else "",
        search_profile.get("creator_lane") if isinstance(search_profile, dict) else "",
        domain_map.get("creator_lane") if isinstance(domain_map, dict) else "",
    ]
    if isinstance(domain_map, dict):
        candidates.extend((domain_map.get("strong_topics") or [])[:2])
    generic = {
        "general", "creator", "content creator", "influencer", "youtube",
        "youtuber", "podcast", "podcaster", "social media",
    }
...
def _build_identity_fingerprint(name: str, link_identity: Dict[str, Any], investigative_dossier: Dict[str, Any], voice_fingerprint: Dict[str, Any]) -> Dict[str, Any]:
    ...
    content_facts = []
    content_facts.extend(businesses[:4])
    content_facts.extend(products[:4])
    content_facts.extend(themes[:4])
    content_facts.extend((voice_fingerprint.get("content_truth") or {}).get("quantified_claims") or [])
    content_facts.extend((voice_fingerprint.get("content_truth") or {}).get("milestones") or [])

    return {
        "bio": ". ".join(_dedupe_keep_order(bio_bits, limit=4)),
        "mission": mission,
        ...
        "verified_facts": _dedupe_keep_order(verified_facts or content_facts, limit=8),
...
def _merge_voice_pattern_packet(voice_fingerprint: Dict[str, Any], packet: Dict[str, Any]) -> Dict[str, Any]:
    ...
    lexical_signature_phrases = clean_style_phrase_list(
    ...
    signature_phrases = _dedupe_keep_order(
        ...
        + lexical_signature_phrases,
        limit=12,
    )
    signature_phrases = clean_style_phrase_list(signature_phrases, limit=12)
    ...
    speech_mechanics["signature_openings"] = _dedupe_keep_order(
        ...
        + clean_style_phrase_list(rhetorical.get("signature_openings") or [], limit=8),
        limit=10,
    )
    ...
    safe_opening_patterns = clean_style_phrase_list(
        list(greeting.get("opening_hooks") or []) + list(rhetorical.get("signature_openings") or []),
        limit=4,
    )
    greeting_mode["opening_move"] = greeting_mode.get("opening_move") or next(iter(safe_opening_patterns or []), "")
...
"""
    ...
    content = rag.generate_chat_completion(
        messages=[
...
class FingerprintService:
    ...
    def __init__(self):
        ...
        self.researcher = GeminiResearchProvider() if _gemini_api_key() else get_research_provider()
    ...
    def _build_creator_profile(creator_id: int, creator: Dict[str, Any], configs: Dict[str, Any], domains: List[str]) -> Dict[str, Any]:
        return {
            ...
            "creator_category": creator.get("creator_category"),
            "creator_archetype": creator.get("creator_archetype"),
        }
    ...
    async def _extract_voice_pattern_packet(
        ...
        if not _gemini_api_key() or not approved_content:
            return {}
        ...
        try:
            parsed = await get_gemini_provider().generate_json_async(
                system_instruction=(
                    ...
                    "Extract behavioral voice patterns, greeting tendencies, rhythm, and rhetorical moves. "
                    "Do not copy transcript hooks, source titles, captions, or video openers into reusable fields; "
    ...
    async def _fetch_url_for_agent(self, url: str, reason: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.get(
                url,
                timeout=8,
                headers={"User-Agent": "Mozilla/5.0 CreatorChat/1.0"},
            )
        ...
        try:
            ...
            creator = db.execute_one(
                ...
                SELECT name, handle, platform_configs, official_domains, course_domains, course_base_urls,
                       ...
                       creator_archetype, archetype_distribution, creator_category
                FROM creators
                WHERE id = %s
                """,
                (creator_id,)
            ...
            public_research_mode = str(getattr(settings, "FINGERPRINT_PUBLIC_RESEARCH", "off") or "off").strip().lower()
            if public_research_mode not in {"on", "true", "1", "full"}:
                policy.enable_link_research = False
                policy.enable_google_expansion = False
                policy.enable_persona_agent = False
                policy.rationale.append("content-first fingerprint: public profile research disabled")
            ...
            if incremental_mode and existing_style_fingerprint and content_corpus_checksum and content_corpus_checksum == fingerprint_corpus_checksum:
                ...
                _set_fingerprint_progress(
                    ...
                    percent=100,
                    stage="complete",
                    message="Creator profile already matches the approved content.",
                )
                return
            ...
            reuse_cached_research = (
                not refresh
                and public_research_mode in {"on", "true", "1", "full"}
            ...
            if reuse_cached_research:
                ...
                if not policy.enable_link_research:
                    ...
                    _set_fingerprint_progress(
                        ...
                        percent=22,
                        stage="content_scan",
                        message="Skipping public profile research and using approved content as the source of truth.",
                    )
                    link_identity = {}
            ...
            approved_content_samples = self._load_approved_content_samples(
                creator_id,
                limit=max(8, min(int(getattr(settings, "PERSONA_AGENT_SAMPLE_LIMIT", 16) or 16), 32)),
            )
            if approved_content_samples:
                ...
                if voice_pattern_packet:
                    ...
                    voice_fingerprint = sanitize_style_fingerprint_for_storage(voice_fingerprint)
            ...
            if not reuse_cached_research and not incremental_mode and policy.enable_persona_agent and policy.enable_google_expansion:
                _set_fingerprint_progress(
                    ...
                    percent=58,
                    stage="persona_agent",
                    message="Checking approved content and public context.",
                )
                ...
                if agentic_bundle:
                    ...
                    if lexical_markers:
                        voice_fingerprint["signature_phrases"] = _dedupe_keep_order(
                            (voice_fingerprint.get("signature_phrases") or [])
                            + clean_style_phrase_list(lexical_markers.get("signature_phrases") or [], limit=8),
                            limit=10,
                        )
                        lexical_rules = voice_fingerprint.get("lexical_rules") or {}
                        lexical_rules["signature_phrases"] = _dedupe_keep_order(
                            (lexical_rules.get("signature_phrases") or [])
                            + clean_style_phrase_list(lexical_markers.get("signature_phrases") or [], limit=8),
                            limit=10,
                        )
                        lexical_rules["high_signal_words"] = _dedupe_keep_order(
            ...
            _set_fingerprint_progress(
                ...
                percent=82,
                stage="synthesis",
                message="Combining voice, worldview, and public facts into one profile.",
            )
            ...
            if incremental_mode and existing_soul_md:
                _set_fingerprint_progress(
                    ...
                    percent=93,
                    stage="finalizing",
                    message="Refreshing the creator profile.",
                )
                ...
                _set_fingerprint_progress(
                    ...
                    percent=93,
                    stage="finalizing",
                    message="Saving the final creator profile for chat.",
                )
            ...
            derived_creator_category = _derive_creator_category_from_fingerprint(voice_fingerprint)
            if derived_creator_category:
                research_summary["search_profile"] = {
                    "creator_category": derived_creator_category,
                    "style_search_profile": voice_fingerprint.get("search_profile") or {},
            ...
            db.execute_update(
                ...
                SET identity_fingerprint = %s,
                    ...
                    research_summary = %s,
                    soul_md = %s,
                    creator_category = CASE WHEN %s <> '' THEN %s ELSE creator_category END,
            ...
            _set_fingerprint_progress(
                ...
                percent=0,
                stage="error",
                message="Creator profile generation failed.",
                error=str(e),
            )
    ...
    async def _generate_soul_artifacts(self, name: str, creator_id: int, research_summary: Dict[str, Any], voice: Dict[str, Any], style_fingerprint: Dict[str, Any]) -> Dict[str, str]:
        ...
        try:
            ...
            return {
                "soul_md": fallback,
                "runtime_prompt_md": (
                    f"You are the {name} creator chat surface. "
                    "Use approved creator content, preserve uncertainty, never mention AI or assistant identity, and never invent private facts."
                ),
            }
