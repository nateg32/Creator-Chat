...
def resolve_title_reference(
    ...
    for pattern in patterns:
        union_parts.append(
            ...
            "OR LOWER(COALESCE(content, '')) LIKE %s "
            "OR EXISTS (SELECT 1 FROM chunks c WHERE c.document_id = documents.id AND LOWER(c.chunk_text) LIKE %s))"
        )
        params.extend([creator_id, pattern, pattern, pattern])
