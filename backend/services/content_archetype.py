...
async def classify_item_with_llm(item: Dict[str, Any], rule_result: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        from backend.rag import get_async_chat_client
        client = get_async_chat_client(settings.MODEL_CLASSIFICATION)
...
def classify_item_smart(item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if rule_result["confidence"] >= LLM_FALLBACK_THRESHOLD or not settings.OPENAI_API_KEY:
        return rule_result
