"""
Central evidence planning for Creator Chat.

Instead of asking only "should I search?", this router decides which world
should answer. Gemini turn-brain decisions can be passed in from the chat
pipeline; when present they are authoritative and this module only converts
that semantic decision into retrieval/search mechanics:
...
_FACTUAL_PATTERNS = [
    ...
    re.compile(r"\bwho(?:'s|\s+is|s)?\s+(?:your|ur|his|her|their)\s+(?:wife|wifey|husband|girlfriend|boyfriend|partner|spouse|missus|misus|mrs|missis)\b", re.IGNORECASE),
    re.compile(r"\b(?:first\s+date|how\s+(?:you|u|he|she|they)\s+(?:met|met\s+your|met\s+his|met\s+her)|where\s+(?:was|were)\s+(?:your|ur|his|her|their)\s+first\s+date)\b", re.IGNORECASE),
    ...
    re.compile(r"\bpublish(?:ed|ing)?\b|\bpublication\b|\brelease(?:d)?\b", re.IGNORECASE),
    re.compile(r"\b(?:when|what\s+year|what\s+date|which\s+month)\b.*\blaunch(?:ed)?\b", re.IGNORECASE),
    re.compile(r"\bfollowers?\b|\bsubscribers?\b|\bmembers?\b|\bstudents?\b|\branking\b|\branked\b|\bvaluation\b|\brevenue\b|\barr\b|\bprofit\b|\bincome\b|\bearnings\b", re.IGNORECASE),
    re.compile(r"\bsales\s+(?:figures?|numbers?|volume|revenue|total|totals|report|reports)\b|\b(?:current|latest|annual|monthly|yearly|last\s+year|this\s+year)\s+sales\b", re.IGNORECASE),
...
_TIMELINE_PATTERNS = [
    re.compile(r"\bwhen (?:did|was|were|is)\b", re.IGNORECASE),
    re.compile(r"\bwhat (?:year|date|month|day)\b", re.IGNORECASE),
    re.compile(r"\bpublish(?:ed|ing)?\b|\bpublication\b|\brelease(?:d)?\b|\bcome out\b", re.IGNORECASE),
    re.compile(r"\b(?:when|what\s+year|what\s+date|which\s+month)\b.*\blaunch(?:ed)?\b", re.IGNORECASE),
...
_STAT_PATTERNS = [
    re.compile(r"\bfollowers?\b|\bsubscribers?\b|\bmembers?\b|\bstudents?\b|\bemployees?\b|\branking\b|\branked\b|\bvaluation\b|\bnet worth\b|\brevenue\b|\barr\b|\bprofit\b|\bincome\b|\bearnings\b", re.IGNORECASE),
    re.compile(r"\bsales\s+(?:figures?|numbers?|volume|revenue|total|totals|report|reports)\b|\b(?:current|latest|annual|monthly|yearly|last\s+year|this\s+year)\s+sales\b", re.IGNORECASE),
]
_LIVE_PATTERNS = [
    re.compile(r"\b(latest|newest|recent|current|today|right now|currently|still|now)\b", re.IGNORECASE),
    re.compile(r"\bfollowers?\b|\bsubscribers?\b|\bprice\b|\bpricing\b|\branking\b|\branked\b|\brevenue\b|\barr\b|\bvaluation\b|\bnet worth\b", re.IGNORECASE),
    re.compile(r"\bsales\s+(?:figures?|numbers?|volume|revenue|total|totals|report|reports)\b|\b(?:current|latest|annual|monthly|yearly|last\s+year|this\s+year)\s+sales\b", re.IGNORECASE),
]
_RESOURCE_PATTERNS = [
    re.compile(r"\bwhere can i (?:buy|get|find|purchase)\b", re.IGNORECASE),
    re.compile(r"\bwatch\b|\blink\b|\bvideo\b|\bepisode\b|\bpodcast\b|\bpost\b|\bclip\b|\breel\b|\bresource\b", re.IGNORECASE),
    re.compile(r"\bwhat\s+(?:do|did|does|was|is).{0,50}\b(?:talk\s+about|cover|discuss|say)\b", re.IGNORECASE),
]
...
_CREATOR_WORLD_HINTS = [
    ...
    re.compile(r"\bnet worth\b|\bemployees\b|\bfounded\b|\bvaluation\b|\bfollowers?\b|\bsubscribers?\b|\brevenue\b|\barr\b|\bprofit\b|\bincome\b|\bearnings\b", re.IGNORECASE),
    re.compile(r"\bsales\s+(?:figures?|numbers?|volume|revenue|total|totals|report|reports)\b|\b(?:current|latest|annual|monthly|yearly|last\s+year|this\s+year)\s+sales\b", re.IGNORECASE),
...
class EvidenceRouter:
    ...
    def _risk_flags(self, query: str, entity: Optional[Dict[str, Any]]) -> List[str]:
        ...
        if policy.kind in {"publication_timeline", "creator_start_timeline"} or explicit_date_request:
            flags.append("date")
        ...
        if policy.kind == "stats" or any(
            ...
            flags.append("stats")
        if any(token in lowered for token in ("latest", "newest", "recent", "current", "today", "right now", "currently", "still", "now")):
            flags.append("fresh")
    ...
    def _smart_value(self, decision: Any, key: str, default: Any = None) -> Any:
        if not decision:
            return default
    ...
    def _smart_confident(self, decision: Any, threshold: float = 0.68) -> bool:
        try:
            return float(self._smart_value(decision, "confidence", 0.0) or 0.0) >= threshold
        except Exception:
            return False

    def _plan_from_smart_decision(
        self,
        query: str,
        resolved_query: str,
        entity: Optional[Dict[str, Any]],
        ...
        *,
        ...
        if not self._smart_confident(smart_decision):
            return None

        query_goal = str(self._smart_value(smart_decision, "query_goal", "") or "").strip().lower()
        if not query_goal or query_goal == "crisis_support":
            return None

        route = str(self._smart_value(smart_decision, "route", "") or "")
        response_mode = str(self._smart_value(smart_decision, "response_mode", "") or "").lower()
        question_type = str(self._smart_value(smart_decision, "question_type", "") or "").lower()
        public_lookup_goals = {
            "resource_lookup",
            "identity_lookup",
            "journey_lookup",
        ...
        smart_creator_fact = bool(self._smart_value(smart_decision, "is_creator_fact", False))
        boundary_can_bypass_retrieval = (
            response_mode == "boundary"
            and query_goal not in public_lookup_goals
            and not smart_creator_fact
        )
        if route in {"ROUTE_0_GREETING", "ROUTE_1_SMALL_TALK"} or response_mode in {"small_talk", "crisis"} or boundary_can_bypass_retrieval:
            return EvidencePlan(
                primary_world="creator_memory",
                secondary_worlds=[],
                should_search_web=False,
                should_search_corpus=False,
                should_verify=False,
                user_is_followup=resolved_query != query,
                resolved_query=resolved_query,
                entity_subject=str(self._smart_value(smart_decision, "entity_subject", "") or ""),
                freshness_required="none",
                answer_mode=response_mode or "answer",
                risk_flags=[],
                query_goal="general",
                search_strategy="turn_brain_no_retrieval",
                entity_type=self._infer_entity_type(resolved_query, entity),
                top_score=top_score,
                contradiction_risk=False,
                plan_version="gemini_turn_brain_v1",
            )

        needs_web = bool(self._smart_value(smart_decision, "needs_web", False))
        needs_corpus = bool(self._smart_value(smart_decision, "needs_corpus", False))
        needs_sources = bool(self._smart_value(smart_decision, "needs_sources", False))
        if query_goal == "general" and question_type in {"domain_advice", "opinion", "meta"} and response_mode in {"answer", "ask_clarifying_question", ""}:
            ...
            return EvidencePlan(
                ...
                user_is_followup=resolved_query != query,
                resolved_query=str(self._smart_value(smart_decision, "resolved_user_message", "") or resolved_query),
                entity_subject=str(self._smart_value(smart_decision, "entity_subject", "") or ""),
        ...
        query_plan = [
            ...
            for item in (self._smart_value(smart_decision, "query_plan", []) or [])
            if str(item or "").strip()
        ][:5]
        smart_resolved_message = str(self._smart_value(smart_decision, "resolved_user_message", "") or "").strip()
        smart_resolved_query = query_plan[0] if query_plan else (smart_resolved_message or resolved_query)
        entity_subject = str(self._smart_value(smart_decision, "entity_subject", "") or "").strip()
        ...
        risk_flags: List[str] = []
        if smart_creator_fact or needs_web or needs_sources:
            risk_flags.append("public_fact")
        if query_goal == "timeline_lookup":
            risk_flags.append("date")
        if query_goal == "price_lookup":
            risk_flags.append("pricing")
        if query_goal in {"stat_lookup", "current_stat_lookup"}:
            risk_flags.append("stats")
        ...
        if query_goal == "current_stat_lookup":
        ...
        elif query_goal in {"stat_lookup", "price_lookup", "timeline_lookup", "role_lookup", "identity_lookup", "journey_lookup", "entity_catalog_lookup"}:
            primary_world = "creator_world"
            freshness_required = "medium" if query_goal in {"stat_lookup", "price_lookup", "timeline_lookup"} else "low"
            search_strategy = "turn_brain_creator_grounded_search"
        elif query_goal in {"availability_lookup", "entity_confirmation", "entity_overview"} and not needs_web:
            primary_world = "creator_memory"
            freshness_required = "none"
            ...
            search_strategy = "turn_brain_grounded_search"
        else:
            primary_world = "creator_memory"
            freshness_required = "none"
            search_strategy = "turn_brain_memory_first"
        ...
        answer_mode = "direct_fact" if query_goal in direct_fact_goals else (
            "resource_recommendation" if query_goal == "resource_lookup" else "creator_take" if question_type == "domain_advice" else "hybrid"
        )
        contradiction = detect_evidence_contradiction(
            smart_resolved_query,
        ...
        return EvidencePlan(
            ...
            user_is_followup=resolved_query != query,
            resolved_query=smart_resolved_query,
            ...
            entity_type=self._infer_entity_type(smart_resolved_query, entity),
            top_score=top_score,
    ...
    def build_plan(
        ...
        smart_plan = self._plan_from_smart_decision(
            query,
            resolved_query,
            entity,
            smart_decision,
