...
NAMED_RESOURCE_PATTERNS = (
    ...
    re.compile(
        r"\bwhat\s+did\s+(?:you|u|ya)\s+"
        r"(?:talk\s+about|cover|discuss|show|go\s+over|say|explain)\s+"
        r"in\s+(?:the\s+)?([^,\.\?\!\n]{4,140})",
        re.IGNORECASE,
    ),
)

CONTENT_SUMMARY_PATTERNS = (
    re.compile(
        r"\b(?:give\s+me|tell\s+me|summari[sz]e|break\s*down|recap|walk\s+(?:me\s+)?through)\s+"
        r"(?:a\s+|the\s+)?(?:deep|full|detailed|complete|proper)?\s*"
        r"(?:breakdown\s+of\s+|summary\s+of\s+|recap\s+of\s+)?"
        r"(?:your\s+|the\s+)?(?:video|podcast|episode|post|reel|short|lesson|resource)\s+"
        r"[\"“]?([^\",\.\?\!\n”]{4,140})",
        re.IGNORECASE,
    ),
    re.compile(
        r"\b(?:summari[sz]e|break\s+down|recap|walk\s+(?:me\s+)?through)\s+"
        r"(?:the\s+)?([^,\.\?\!\n]{4,140})",
        re.IGNORECASE,
    ),
    re.compile(
        r"\b(?:what\s+(?:are|were)|list)\s+(?:the\s+)?"
        r"([^,\.\?\!\n]{4,140})",
        re.IGNORECASE,
    ),
)
...
GENERIC_QUERY_TERMS = {
    ...
    "find", "posted", "write", "wrote", "on", "did", "was", "each", "every", "all",
    "one", "ones", "them",
}

SUMMARY_REQUEST_TERMS = {
    "summarize", "summarise", "summary", "recap", "breakdown", "break", "list",
    "covered", "covers", "takeaways", "points", "steps", "habits", "lessons",
}


def is_content_summary_request(question: str) -> bool:
    """Return True when the user wants the answer extracted from creator content, not a link."""
    ...
    return bool(words & SUMMARY_REQUEST_TERMS and {"video", "episode", "podcast", "lesson", "post", "reel", "short"} & words)
...
def extract_named_resource_fragments(question: str) -> List[str]:
    ...
    seen = set()
    ordered: List[str] = []
    patterns = list(NAMED_RESOURCE_PATTERNS)
    if is_content_summary_request(text):
        patterns.extend(CONTENT_SUMMARY_PATTERNS)
    for pattern in patterns:
        for fragment in pattern.findall(text):
            clean = re.sub(r"\s+", " ", str(fragment or "")).strip(" .,:;!?\"'")
            clean = re.split(
                r"\b(?:give\s+me|tell\s+me|show\s+me|list)\s+(?:each|every|all|the|them|one)\b",
                clean,
                maxsplit=1,
                flags=re.IGNORECASE,
            )[0].strip(" .,:;!?\"'")
            ...
            if re.fullmatch(
                r"(?:a\s+|the\s+)?(?:deep|full|detailed|complete|proper)?\s*"
                r"(?:breakdown|summary|recap|takeaways?|main points|key points|lessons?)",
                clean,
                flags=re.IGNORECASE,
            ):
                continue
            if len([w for w in re.findall(r"[a-z0-9']+", clean.lower()) if w not in GENERIC_QUERY_TERMS]) < 2:
                continue
            key = clean.lower()
            if key in seen:
                continue
