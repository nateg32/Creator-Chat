...
class AskRequest(BaseModel):
    ...
    images: Optional[List[ImageInput]] = None  # Attached image
    debug: Optional[bool] = False

    @validator("images")
    def _limit_images(cls, value):
        if value is not None and len(value) > 1:
            raise ValueError("At most one image can be attached.")
        return value
...
class SearchedItem(BaseModel):
    ...
    raw_review_status: Optional[str] = None
    is_primary: Optional[bool] = True
    duplicate_of_item_id: Optional[str] = None
    covered_by_existing_document: Optional[bool] = False
    hidden_from_review: Optional[bool] = False
