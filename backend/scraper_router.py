...
import sys
import os
# Since worker runs from root 'Creator Chat', we need 'backend' in path
...
def _platform_label(key: str) -> str:
    ...
    return (plat or {}).get("label") or key.replace("_", " ").title()


def _status_payload(key: str, status: str, *, error: Optional[str] = None, items_found: int = 0) -> Dict[str, Any]:
    return {
...
def _invalid_url_message(key: str, err: Optional[str] = None) -> str:
    label = _platform_label(key)
    base = f"Link format is incorrect. Use the correct {label} profile URL format."
    if err:
        return f"{base} {err}"
    return base


def _empty_result_message(key: str) -> str:
    label = _platform_label(key)
    return f"No public content found for this {label} profile. Check that the link is correct, public, and has content available to scrape."
...
def run_search_router(
    ...
    def scrape_one(key: str, cfg: Dict[str, Any]):
        ...
        if not url:
            with results_lock:
                ...
                platform_statuses[key] = _status_payload(key, "skipped", error=f"Add a {_platform_label(key)} profile URL before searching.")
        ...
        if not plat:
            with results_lock:
                completed_counter[0] += 1
                platform_statuses[key] = _status_payload(key, "skipped", error="Unknown platform.")
            if progress_callback:
                progress_callback(key, "skipped", completed_counter[0], total_platforms)
            return
        ...
        if not ok:
            with results_lock:
                completed_counter[0] += 1
                platform_statuses[key] = _status_payload(key, "error", error=_invalid_url_message(key, err))
            if progress_callback:
        ...
        if not mapper:
            with results_lock:
                completed_counter[0] += 1
                platform_statuses[key] = _status_payload(key, "skipped", error=f"{_platform_label(key)} search is not implemented yet.")
            if progress_callback:
                progress_callback(key, "skipped", completed_counter[0], total_platforms)
            return
        ...
        try:
            ...
            with results_lock:
                ...
                platform_statuses[key] = _status_payload(
                    key,
                    "success" if item_count else "empty",
                    error=None if item_count else _empty_result_message(key),
            ...
            with results_lock:
                ...
                platform_statuses[key] = _status_payload(
                    key,
                    "error",
                    error=str(e) or f"{_platform_label(key)} search failed. Check the link and try again.",
                )
            if progress_callback:
                progress_callback(key, "error", completed_counter[0], total_platforms)
    ...
    empty = sum(1 for s in platform_statuses.values() if s.get("last_scrape_status") == "empty")
    total_found = sum(s.get("items_found", 0) for s in platform_statuses.values())
    
    print(f"[SEARCH] Parallel Run Summary: {successful} succeeded, {empty} empty, {failed} failed, {skipped} skipped")
