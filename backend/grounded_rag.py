...
try:
    from backend.services.crisis_intent import build_crisis_response, detect_crisis_followup_intent, detect_crisis_intent
except Exception:
    def detect_crisis_intent(*args, **kwargs):
        return None

    def detect_crisis_followup_intent(*args, **kwargs):
        return None

    def build_crisis_response(*args, **kwargs):
        return "Please contact emergency services or a crisis line right now."
...
try:
    ...
    def detect_message_vibe(*args, **kwargs):
        return {}

    def format_vibe_prompt_block(*args, **kwargs):
        return ""
try:
    from backend.services.thread_context_cache import thread_context_cache
except Exception:
    class _NoopThreadContextCache:
        def get_reusable_context(self, **kwargs):
            return None
...
_TITLE_CATALOG_CACHE: Dict[int, Tuple[float, List[str]]] = {}
_TITLE_CATALOG_TTL_SECONDS = 120.0
_TITLE_CATALOG_LIMIT = 80
_PURE_SOCIAL_FILLER = {"bro", "bruh", "bruv", "man", "mate", "ma", "dude", "g", "fam", "chief", "homie"}
_ELONGATED_GREETING_TOKEN_RE = re.compile(
    r"^(?:h+i+|he+y+|hello+|y+o+|sup+|wass+up+|wsg+|hiya+)$",
    re.IGNORECASE,
)
_SOCIAL_TASK_HINT_RE = re.compile(
    r"\b("
    r"what|whats|who|when|where|why|how|do|did|does|can|should|tell|explain|"
    r"trade|trading|setup|chart|business|gym|workout|start|help|recommend|advice|plan|"
    r"source|video|link|post|show|send"
    r")\b",
    re.IGNORECASE,
)
...
def _squash_repeated_letters(value: str) -> str:
    return re.sub(r"(.)\1{2,}", r"\1\1", str(value or "").lower())
...
def _is_social_filler_token(token: str) -> bool:
    ...
    if not cleaned:
        return False
    if cleaned in _PURE_SOCIAL_FILLER:
        return True
...
def _is_pure_social_greeting(question: str) -> bool:
    """True for bare greetings like "yo", "yoo", "hey bro"; never a task."""
    ...
    if is_greeting(raw):
        return True
    if _SOCIAL_TASK_HINT_RE.search(" ".join(tokens[1:])):
        return False
    has_greeting = any(_ELONGATED_GREETING_TOKEN_RE.fullmatch(token) for token in tokens)
    if not has_greeting:
        return False
    return all(
        _ELONGATED_GREETING_TOKEN_RE.fullmatch(token) or _is_social_filler_token(token)
        for token in tokens
    )


_LIGHT_CLARIFICATION_TURNS = {
    "huh",
    "what",
    "wut",
    "wdym",
...
def _is_light_clarification_turn(question: str) -> bool:
    ...
    if normalized in _LIGHT_CLARIFICATION_TURNS or compact in _LIGHT_CLARIFICATION_TURNS:
        return True
    words = normalized.split()
    return len(words) <= 3 and any(word in {"huh", "what", "wut"} for word in words)
...
def _quality_markers_for_creator(creator_profile: Dict[str, Any]) -> List[str]:
    style_fp = sanitize_style_fingerprint_for_runtime(_coerce_json_dict((creator_profile or {}).get("style_fingerprint")))
    voice_profile = sanitize_voice_profile_for_runtime(_coerce_json_dict((creator_profile or {}).get("voice_profile")))
    ...
    for value in (
        ...
        + clean_style_phrase_list(lexical_rules.get("signature_phrases") or [], limit=4)
        + clean_style_phrase_list(voice_profile.get("signature_phrases") or [], limit=4)
    ):
...
def _build_evidence_plan(
    ...
    *,
    ...
    smart_decision: Optional[Any] = None,
...
def _should_route_personal_fact_question(
    ...
    *,
    ...
    if _is_user_memory_question(question):
        return False
...
def _should_prompt_only_personal_fact(decision: Any) -> bool:
    """Use the normal Gemini prompt stream for creator bio questions that do not need verified web work."""
    if not _smart_decision_confident(decision):
        return False
    question_type = str(_smart_decision_value(decision, "question_type", "") or "").lower()
    query_goal = str(_smart_decision_value(decision, "query_goal", "") or "").lower()
    needs_web = bool(_smart_decision_value(decision, "needs_web", False))
    needs_sources = bool(_smart_decision_value(decision, "needs_sources", False))
    if needs_web or needs_sources:
        return False
    if question_type not in {"personal_bio", "creator_fact"} and not bool(_smart_decision_value(decision, "is_creator_fact", False)):
        return False
    return query_goal in {"", "general", "journey_lookup", "identity_lookup", "role_lookup"}


_USER_MEMORY_QUESTION_RE = re.compile(
    r"\b("
    ...
    r"what\s+do\s+i\s+(?:like|prefer|want|play|do)|"
    r"what\s+(?:are\s+)?my\s+(?:goals?|preferences?|interests?|plans?|sport|sports)"
    r")\b",
    re.IGNORECASE,
)


def _is_user_memory_question(question: str) -> bool:
    lowered = str(question or "").strip().lower()
    if not lowered:
        return False
    return bool(_USER_MEMORY_QUESTION_RE.search(lowered))


def _smart_decision_value(decision: Any, key: str, default: Any = None) -> Any:
    if not decision:
        return default
...
def _smart_decision_confident(decision: Any, threshold: float = 0.68) -> bool:
    try:
        return float(_smart_decision_value(decision, "confidence", 0.0) or 0.0) >= threshold
    except Exception:
        return False


def _smart_source_policy(decision: Any) -> str:
    policy = str(_smart_decision_value(decision, "source_policy", "") or "").strip().lower()
    if policy in {"none", "cite_if_used", "attach_resource", "must_cite"}:
        return policy
    if not _smart_decision_confident(decision):
        return ""
    route = str(_smart_decision_value(decision, "route", "") or "")
    response_mode = str(_smart_decision_value(decision, "response_mode", "") or "").lower()
    query_goal = str(_smart_decision_value(decision, "query_goal", "") or "").lower()
    ...
    if bool(_smart_decision_value(decision, "needs_sources", False)):
        return "must_cite"
    return "none"


def _smart_turn_context_note(decision: Any, question: str) -> str:
    if not _smart_decision_confident(decision, threshold=0.62):
        return ""
    parts: List[str] = []
    resolved = re.sub(
        r"\s+",
        " ",
        str(_smart_decision_value(decision, "resolved_user_message", "") or "").strip(),
    )
    if resolved and resolved.lower() != str(question or "").strip().lower():
        parts.append(f"Meaning: {resolved[:420]}")
    source_policy = _smart_source_policy(decision)
    if source_policy == "none":
        parts.append("Source handling: answer directly; do not mention links, attachments, citations, or sources unless the user explicitly asked.")
    elif source_policy == "cite_if_used":
        parts.append("Source handling: only show sources if the answer explicitly relies on a specific source.")
    elif source_policy == "attach_resource":
        parts.append("Source handling: a resource can be attached if it directly helps; explain why it is useful before mentioning the attachment.")
    elif source_policy == "must_cite":
        parts.append("Source handling: verify public facts and keep source cards/citations aligned with facts actually used.")
    return " ".join(parts)


def _answer_mentions_attachment(answer_text: str) -> bool:
    text = str(answer_text or "").lower()
    return bool(
        re.search(r"\b(attached|attach(?:ing)?|linked|link below|source below|video below|resource below)\b", text)
        or re.search(r"\b(i|we)\s+(?:put|dropped|included)\s+(?:it|that|the link|the video|the resource)\s+below\b", text)
    )


def _answer_uses_citation_markers(answer_text: str) -> bool:
    return bool(re.search(r"\[\s*\d+\s*\]", str(answer_text or "")))


def _should_build_source_cards_for_turn(
    decision: Any,
    *,
    answer_text: str,
    ...
    video_policy: str = "",
    has_substantive_rag: bool = False,
) -> bool:
    explicit_resource_need = bool(wants_link or is_video_request or should_run_recommender)
    if str(video_policy or "").lower() == "forced":
        explicit_resource_need = True
    if explicit_resource_need:
        return True
    if _answer_mentions_attachment(answer_text):
        return True

    source_policy = _smart_source_policy(decision)
    if source_policy == "attach_resource":
        return True
    if source_policy in {"none", "cite_if_used"}:
        return False
    if source_policy == "must_cite":
        # Must-cite turns should emit citations. Only build preview cards when
        # the answer is actually presenting a resource below.
        return False
    if not _smart_decision_confident(decision):
        return bool(has_substantive_rag and _answer_mentions_attachment(answer_text))
    return False


def _should_emit_citations_for_turn(
    decision: Any,
    *,
    answer_text: str,
    wants_link: bool = False,
    ...
    if source_policy == "none":
        return False
    if source_policy in {"attach_resource", "must_cite"}:
        return True
    if source_policy == "cite_if_used":
        return bool(_answer_uses_citation_markers(answer_text) or _answer_mentions_attachment(answer_text))
    if not _smart_decision_confident(decision):
        return bool(wants_link or is_video_request or should_run_recommender or _answer_uses_citation_markers(answer_text))
    return False


def _smart_decision_allows_identity_shortcut(decision: Any) -> bool:
    if not _smart_decision_confident(decision):
        return True
    question_type = str(_smart_decision_value(decision, "question_type", "") or "").lower()
    query_goal = str(_smart_decision_value(decision, "query_goal", "") or "").lower()
    intent = str(_smart_decision_value(decision, "intent", "") or "").lower()
    if question_type in {"personal_bio", "creator_fact"}:
        return False
    if query_goal in {"journey_lookup", "timeline_lookup", "role_lookup", "identity_lookup"}:
        return False
    if any(token in intent for token in ("background", "journey", "story", "rich", "business_history")):
        return False
    return True


def _smart_decision_is_crisis(decision: Any) -> bool:
    if not _smart_decision_confident(decision, threshold=0.72):
        return False
    question_type = str(_smart_decision_value(decision, "question_type", "") or "").lower()
    response_mode = str(_smart_decision_value(decision, "response_mode", "") or "").lower()
    intent = str(_smart_decision_value(decision, "intent", "") or "").lower()
    return bool(
        ...
        or (response_mode == "crisis" and "self" in intent and "harm" in intent)
        or intent in {"self_harm", "suicidal_ideation", "crisis_self_harm"}
    )


def _smart_decision_requires_boundary(decision: Any) -> bool:
    if not _smart_decision_confident(decision, threshold=0.68):
        return False
    question_type = str(_smart_decision_value(decision, "question_type", "") or "").lower()
    response_mode = str(_smart_decision_value(decision, "response_mode", "") or "").lower()
    intent = str(_smart_decision_value(decision, "intent", "") or "").lower()
    return bool(
        ...
        or question_type in {"harmful_request", "out_of_scope", "private_sensitive"}
        or any(token in intent for token in ("harmful", "unsafe", "illegal", "malicious", "out_of_scope"))
    )


def _build_smart_boundary_response(
    *,
    question: str,
    ...
    bridge_topic = recent_bridge_topic(conversation_history or [], question)
...
def _smart_intent_router_available():
    try:
        from backend.services.smart_intent_router import smart_intent_router, should_use_smart_router
        return smart_intent_router, should_use_smart_router
    except Exception as exc:
        logger.info("Smart intent router unavailable: %s", exc)
        return None, None


def _apply_smart_routing_decision(
    decision: Any,
    *,
    route: str,
    route_q_type: str,
    ...
    if not _smart_decision_confident(decision):
        return route, route_q_type, route_topic, rule_intent, False

    smart_route = str(_smart_decision_value(decision, "route", "") or "")
    smart_type = str(_smart_decision_value(decision, "question_type", "") or "").lower()
    smart_intent = str(_smart_decision_value(decision, "intent", "") or "").lower()
    smart_goal = str(_smart_decision_value(decision, "query_goal", "") or "").lower()
    is_creator_fact = bool(_smart_decision_value(decision, "is_creator_fact", False))

    if smart_route in {"ROUTE_0_GREETING", "ROUTE_1_SMALL_TALK", "ROUTE_2_TASK"}:
        route = smart_route
    if smart_type:
        route_q_type = "personal_bio" if smart_type == "creator_fact" else smart_type
    if smart_type == "self_harm":
        route = "ROUTE_2_TASK"
        route_q_type = "self_harm"
        rule_intent = "self_harm"
    elif smart_type == "user_memory" or smart_intent == "user_memory":
        route = "ROUTE_2_TASK"
        route_q_type = "user_memory"
        rule_intent = "user_memory"
    elif is_creator_fact or smart_goal in {
        "journey_lookup",
        "timeline_lookup",
        "role_lookup",
        "identity_lookup",
        ...
        rule_intent = "personal_bio_question"
    elif smart_route == "ROUTE_1_SMALL_TALK" or smart_type == "small_talk":
        route = "ROUTE_1_SMALL_TALK"
        route_q_type = "greeting"

    logger.info(
        "Smart intent routing applied intent=%s route=%s q_type=%s goal=%s confidence=%s reason=%s",
        smart_intent,
        route,
        route_q_type,
        smart_goal,
        _smart_decision_value(decision, "confidence", 0),
        _smart_decision_value(decision, "reason", ""),
    )
    return route, route_q_type, route_topic, rule_intent, True


def _apply_smart_evidence_plan(
    decision: Any,
    ...
    *,
    ...
    needs_web = bool(_smart_decision_value(decision, "needs_web", False))
    needs_sources = bool(_smart_decision_value(decision, "needs_sources", False))
    needs_corpus = bool(_smart_decision_value(decision, "needs_corpus", False))
    if query_goal in {"", "general"}:
        if question_type in {"domain_advice", "opinion", "meta"} and response_mode in {"", "answer", "ask_clarifying_question"}:
            from dataclasses import replace

            resolved_query = str(_smart_decision_value(decision, "resolved_user_message", "") or "").strip() or question
            primary_world = "live_world" if needs_web else "creator_memory"
            freshness_required = "low" if needs_web else "none"
            search_strategy = "turn_brain_general_web" if needs_web else "turn_brain_domain_advice"
            risk_flags = ["public_fact"] if needs_web or needs_sources else []
            if plan:
                return replace(
                    plan,
                    query_goal="general",
                    ...
                    should_search_corpus=needs_corpus,
                    should_verify=needs_sources,
                    entity_subject="",
                    ...
                    plan_version=f"{plan.plan_version}+gemini_general",
                )
            return EvidencePlan(
                primary_world=primary_world,
                secondary_worlds=[],
                ...
                should_search_corpus=needs_corpus,
                should_verify=needs_sources,
                user_is_followup=resolved_query != question,
                resolved_query=resolved_query,
                entity_subject="",
                freshness_required=freshness_required,
                answer_mode="creator_take",
                ...
                entity_type="",
                contradiction_risk=False,
                plan_version="gemini_turn_brain_v1",
            )
        return plan
    ...
    try:
        ...
        entity_subject = str(_smart_decision_value(decision, "entity_subject", "") or "").strip()
        query_plan = [
            str(item).strip()
            for item in (_smart_decision_value(decision, "query_plan", []) or [])
            if str(item or "").strip()
        ][:5]
        resolved_message = str(_smart_decision_value(decision, "resolved_user_message", "") or "").strip()
        resolved_query = query_plan[0] if query_plan else (resolved_message or question)
        risk_flags = ["public_fact"] if needs_web or needs_sources else []
        if query_goal in {"timeline_lookup", "price_lookup", "stat_lookup", "current_stat_lookup"}:
            risk_flags.append("date" if query_goal == "timeline_lookup" else "stats")
        if query_goal == "current_stat_lookup":
            risk_flags.append("fresh")
        if query_goal == "resource_lookup":
            risk_flags.append("resource_request")

        if query_goal == "current_stat_lookup":
            primary_world = "live_world"
            freshness_required = "high"
            answer_mode = "direct_fact"
            search_strategy = "turn_brain_live_grounded_search"
        elif query_goal in {"timeline_lookup", "price_lookup", "stat_lookup", "role_lookup", "identity_lookup"}:
            primary_world = "creator_world"
            freshness_required = "medium" if query_goal in {"timeline_lookup", "price_lookup", "stat_lookup"} else "low"
            answer_mode = "direct_fact"
            search_strategy = "turn_brain_creator_grounded_search"
        elif query_goal == "journey_lookup":
            primary_world = "creator_world"
            freshness_required = "low"
            answer_mode = "hybrid"
            search_strategy = "turn_brain_journey_grounded_search"
        elif query_goal == "resource_lookup":
            primary_world = "creator_world" if needs_web else "creator_memory"
            freshness_required = "low" if needs_web else "none"
            answer_mode = "resource_recommendation"
            search_strategy = "turn_brain_resource_web_plus_corpus" if needs_web else "turn_brain_resource_corpus"
        elif query_goal in {"entity_confirmation", "entity_overview", "availability_lookup"} and not needs_web:
            primary_world = "creator_memory"
            freshness_required = "none"
            answer_mode = "hybrid"
            search_strategy = "turn_brain_entity_graph_first"
        else:
            primary_world = "creator_world" if needs_web else "creator_memory"
            freshness_required = "low" if needs_web else "none"
            answer_mode = "hybrid"
            search_strategy = "turn_brain"

        secondary_worlds = []
        if needs_corpus and primary_world != "creator_memory":
            secondary_worlds.append("creator_memory")
        if needs_web and primary_world != "live_world" and query_goal == "current_stat_lookup":
            secondary_worlds.append("live_world")
        if plan:
            return replace(
                plan,
                ...
                should_search_web=needs_web,
                should_search_corpus=needs_corpus,
                should_verify=needs_sources or needs_web,
                entity_subject=entity_subject or plan.entity_subject,
                resolved_query=resolved_query or plan.resolved_query,
                risk_flags=list(dict.fromkeys([*(plan.risk_flags or []), *risk_flags])),
                answer_mode=answer_mode,
                search_strategy=search_strategy,
                freshness_required=freshness_required,
                plan_version=f"{plan.plan_version}+smart_intent",
            )
        return EvidencePlan(
            primary_world=primary_world,
            secondary_worlds=secondary_worlds,
            ...
            search_strategy=search_strategy,
            entity_type="",
            plan_version="smart_intent_router_v1",
        )
    except Exception as exc:
        logger.info("Smart evidence plan merge skipped: %s", exc)
        return plan
...
def _personal_answer_or_fallback(answer: Any, question: str, creator_row: Optional[Dict[str, Any]]) -> str:
    ...
    return "I do not have that verified cleanly enough to answer it as a fact."
...
def _rescue_cards_from_answer(
    ...
    for chunk in support_set:
        ...
        direct_title_mention = bool(normalized_title and normalized_title in normalized_answer)
        hits = sum(1 for w in title_words if w in answer_lower)
        overlap = hits / len(title_words)
        # Prefer an exact title phrase, but still rescue close title mentions.
        if direct_title_mention:
            overlap = max(overlap, 1.0)
        if (direct_title_mention or (overlap >= 0.35 and hits >= 2)) and overlap > best_overlap:
...
def _align_response_cards_with_answer(
    answer_text: str,
    support_set: Optional[List[Dict[str, Any]]],
    cards: Optional[List[Dict[str, str]]],
) -> List[Dict[str, str]]:
    """Put a card for the title named in the final answer first.

    This prevents the prose from saying it attached one source while the UI shows
    a neighboring retrieval result.
    """
    existing_cards = list(cards or [])
    mentioned_cards = _rescue_cards_from_answer(answer_text, support_set)
    if not mentioned_cards:
        return existing_cards

    aligned: List[Dict[str, str]] = []
    seen_urls: Set[str] = set()
    for card in mentioned_cards + existing_cards:
        ...
        aligned.append(card)
        seen_urls.add(key)
    return aligned[: max(1, len(existing_cards))]
...
_SEARCH_FOLLOWUP_CONTEXT_SIGNALS = re.compile(
...
_VIDEO_RESOURCE_REQUEST_RE = re.compile(
    r"\b(?:video|videos|watch|youtube|yt|reel|reels|short|shorts|clip|clips|tutorial|tiktok)\b",
    re.IGNORECASE,
)
_NON_VIDEO_RESOURCE_CONTEXT_RE = re.compile(
    r"\b(?:podcast|episode|article|book|newsletter|website|web\s?page|page|spotify|apple\s+podcasts?|audible)\b",
    re.IGNORECASE,
)
_VIDEO_RESOURCE_CONTEXT_RE = re.compile(
    r"\b(?:video|youtube|watch|reel|short|clip|tiktok|facebook\s+watch)\b",
    re.IGNORECASE,
)
_SEARCH_FOLLOWUP_STOP_WORDS = {
...
def _search_context_terms(text: str) -> set:
    return {
        word
        for word in re.findall(r"[a-z0-9']+", str(text or "").lower())
        if len(word) > 2 and word not in _SEARCH_FOLLOWUP_STOP_WORDS
    }


def _assistant_context_sentence_for_query(
    question: str,
    history: Optional[List[Dict[str, str]]] = None,
) -> str:
    """Use assistant-introduced anchors for short follow-up retrieval queries."""
    if not history:
        return ""
    question_terms = _search_context_terms(question)
    pronoun_followup = bool(re.search(r"\b(?:it|that|this|one|thing)\b", str(question or "").lower()))
    resource_followup = bool(_RESOURCE_FOLLOWUP_RE.search(str(question or "")))
    if not question_terms and not pronoun_followup and not resource_followup:
        return ""
    ...
    for msg in reversed(history[-6:]):
        ...
        for sentence in re.split(r"(?<=[.!?])\s+", text):
            ...
            if not sentence or not _SEARCH_FOLLOWUP_CONTEXT_SIGNALS.search(sentence):
                continue
            sentence_terms = _search_context_terms(sentence)
            ...
            if resource_followup and _RESOURCE_CONTEXT_RE.search(sentence):
                score += 2
            if len(sentence) > 320:
                sentence = sentence[:320].rsplit(" ", 1)[0].strip()
            if score > best_score:
                best = sentence
                best_score = score
        if best:
...
def _latest_assistant_text(history: Optional[List[Dict[str, str]]] = None) -> str:
...
def _recent_assistant_resource_sentence(last_bot_msg: str) -> str:
    text = re.sub(r"\s+", " ", str(last_bot_msg or "")).strip()
    if not text:
        return ""
    for sentence in reversed(re.split(r"(?<=[.!?])\s+", text)):
        sentence = re.sub(r"\s+", " ", sentence).strip()
        if sentence and _RESOURCE_CONTEXT_RE.search(sentence):
            return sentence
    return ""


def _followup_resource_targets_non_video(question: str, last_bot_msg: str) -> bool:
    """Treat "link the video" as "link that resource" when the prior anchor was a podcast/article."""
    if not _RESOURCE_FOLLOWUP_RE.search(question or ""):
        return False
    sentence = _recent_assistant_resource_sentence(last_bot_msg)
    if not sentence:
        return False
    if _NON_VIDEO_RESOURCE_CONTEXT_RE.search(sentence) and not _VIDEO_RESOURCE_CONTEXT_RE.search(sentence):
        return True
    return False


def _followup_resource_requires_video(question: str, last_bot_msg: str) -> bool:
    if _followup_resource_targets_non_video(question, last_bot_msg):
        return False
    if _VIDEO_RESOURCE_REQUEST_RE.search(question or ""):
        return True
    sentence = _recent_assistant_resource_sentence(last_bot_msg)
    return bool(sentence and _VIDEO_RESOURCE_CONTEXT_RE.search(sentence))
...
def build_search_query(question: str, history: Optional[List[Dict[str, str]]] = None) -> str:
    ...
    if history and is_follow_up_requesting_links(question, history):
        ...
        if len(q_words) <= 7:
            assistant_context = _assistant_context_sentence_for_query(question, history)
            if assistant_context:
                query_parts.append(assistant_context)
...
def fetch_all_document_titles(creator_id: int, limit: int = _TITLE_CATALOG_LIMIT) -> List[str]:
    """Return a bounded, briefly cached title catalog for source-aware turns."""
    now = time.monotonic()
    cached = _TITLE_CATALOG_CACHE.get(int(creator_id))
    if cached and now - cached[0] <= _TITLE_CATALOG_TTL_SECONDS:
        return list(cached[1])

    safe_limit = max(1, min(int(limit or _TITLE_CATALOG_LIMIT), 200))
    rows = db.execute_query(
        """
        SELECT DISTINCT title
        FROM documents
        WHERE creator_id = %s
          AND (metadata->>'type' IS NULL OR metadata->>'type' != 'persona')
          AND title IS NOT NULL
          AND title != ''
        ORDER BY title
        LIMIT %s
        """,
        (creator_id, safe_limit),
    )
    titles = [r["title"] for r in rows if r.get("title")]
    _TITLE_CATALOG_CACHE[int(creator_id)] = (now, titles)
    return list(titles)


_TITLE_CATALOG_TRIGGER_RE = re.compile(
    r"\b(video|videos|episode|episodes|clip|clips|reel|reels|post|posts|"
    r"watch|source|sources|resource|resources|link|links|title|titles|"
    r"where\s+(?:do|did)\s+you\s+(?:talk|cover|say|mention)|"
    r"what\s+did\s+you\s+(?:talk|say|cover|mention)|"
    r"which\s+(?:video|episode|clip|post|reel))\b",
    re.IGNORECASE,
)


def _should_fetch_title_catalog_for_turn(
    question: str,
    *,
    wants_link: bool = False,
    is_video_request: bool = False,
    should_run_recommender: bool = False,
    content_summary_request: bool = False,
    support_set: Optional[List[Dict[str, Any]]] = None,
) -> bool:
    if wants_link or is_video_request or should_run_recommender or content_summary_request:
        return True
    if extract_named_resource_fragments(question):
        return True
    if _TITLE_CATALOG_TRIGGER_RE.search(question or ""):
        return True
    return False


def _should_use_gemini_context_cache_for_turn(
    question: str,
    support_set: Optional[List[Dict[str, Any]]],
    *,
    wants_link: bool = False,
    ...
    if not getattr(settings, "GEMINI_DYNAMIC_RAG_ENABLED", False):
        return False
    q = (question or "").strip()
    if not q:
        return False
    top_score = _support_set_top_score(support_set) or 0.0
    exact_reference = bool(
        extract_named_resource_fragments(q)
        or re.search(r"\b(timestamp|quote|exact|verbatim|where\s+(?:in|did|do)|what\s+did\s+you\s+(?:say|talk|cover|mention)|which\s+(?:video|episode|clip|post|reel))\b", q, re.IGNORECASE)
    )
    if content_summary_request or exact_reference:
        return top_score < 0.78
    if wants_link or is_video_request or should_run_recommender:
...
def retrieve_candidates(
    ...
    if query_embedding is None:
        logger.info("retrieve_candidates skipped because query_embedding is missing.")
        return []
    try:
        embedding_values = list(query_embedding)
    except TypeError:
        logger.warning("retrieve_candidates skipped because query_embedding is not iterable.")
        return []
    if not embedding_values:
        logger.info("retrieve_candidates skipped because query_embedding is empty.")
        return []
...
def _build_not_online_fallback(
    ...
    sfp = sanitize_style_fingerprint_for_runtime(style_fingerprint or {})
    lexical = sfp.get("lexical_rules") or {}
    sig_phrases = clean_style_phrase_list(lexical.get("signature_phrases") or [], limit=3)
    ...
    if sig_phrases or boundary_rules:
        try:
            ...
            voice_hint = ""
            if sig_phrases:
                voice_hint = f"Use these vocabulary pattern cues without copying them: {', '.join(sig_phrases[:2])}. "
            if boundary_rules:
...
def _creator_search_terms(creator_row: Dict[str, Any]) -> Dict[str, str]:
    ...
    style_fp = _coerce_json_dict(creator_row.get("style_fingerprint"))
    search_profile = _coerce_json_dict(style_fp.get("search_profile"))
    domain_map = _coerce_json_dict(style_fp.get("domain_map"))
    niche = str(
        search_profile.get("primary_category")
        or search_profile.get("creator_lane")
        or creator_row.get("creator_category")
        or creator_row.get("creator_archetype")
        or domain_map.get("creator_lane")
        or ""
    ).strip()
    search_identity_terms = [
        ...
        for term in (
            ...
            + list(search_profile.get("disambiguation_terms") or [])
            + list(domain_map.get("strong_topics") or [])[:3]
        )
        if str(term or "").strip()
    ]
    search_identity_terms = list(dict.fromkeys(search_identity_terms))[:8]
    if not niche and search_identity_terms:
        niche = " ".join(search_identity_terms[:4])
    if not niche:
    ...
    return {
        ...
        "handle": handle,
        "niche": niche,
        "identity_terms": " ".join(search_identity_terms[:6]),
    }


def _hydrate_creator_search_profile(creator_row: Dict[str, Any]) -> Dict[str, Any]:
    """
    Live search needs creator identity signals, not just the display name.
    Hydrate only on the web-search path so normal chat turns do not pay this DB cost.
    """
    if not creator_row or not creator_row.get("id"):
        return creator_row or {}
    desired_columns = [
        "platform_configs",
        "youtube_handle",
        "youtube_channel_id",
    ...
    if all(column in creator_row for column in desired_columns):
        return creator_row
    try:
        select_parts = ["id"] + [_creator_select_expr(column) for column in desired_columns]
        row = db.execute_one(
            f"SELECT {', '.join(select_parts)} FROM creators WHERE id = %s LIMIT 1",
            (creator_row.get("id"),),
        ) or {}
    except Exception as exc:
        logger.warning("Creator search profile hydration skipped: %s", exc)
        return creator_row
    hydrated = dict(creator_row)
    for key, value in row.items():
        if key != "id" and key not in hydrated:
            hydrated[key] = value
        elif key != "id" and hydrated.get(key) in (None, "", {}, []):
            hydrated[key] = value
    return hydrated
...
def _is_strict_rag_mode(creator_row: Optional[Dict[str, Any]]) -> bool:
    return _normalize_creator_search_mode((creator_row or {}).get("search_mode")) == "ingested_only"


def _is_live_or_cached_external_chunk(chunk: Dict[str, Any]) -> bool:
...
def _filter_strict_rag_support(support_set: Optional[List[Dict[str, Any]]]) -> List[Dict[str, Any]]:
    return [
        chunk
        for chunk in (support_set or [])
        if isinstance(chunk, dict) and not _is_live_or_cached_external_chunk(chunk)
    ]
...
def _image_turn_wants_creator_sources(question: str) -> bool:
    text = (question or "").lower()
    triggers = [
        "source", "sources", "link", "links", "video", "clip", "reel", "post",
        "where do you", "where did you", "have you talked", "do you have",
        "which video", "send me", "show me", "content about",
    ]
...
def _decision_is_creator_public_fact(decision: Optional[SearchDecision]) -> bool:
    ...
    return decision.reason in {
        ...
        "no_entity_support",
        "turn_brain_public_fact",
        "turn_brain_live_fact",
    }
...
def _run_live_web_search(
    ...
    creator_row = _hydrate_creator_search_profile(creator_row)
    if _is_strict_rag_mode(creator_row):
        logger.info("Strict RAG mode active; live web search blocked.")
        return []
    ...
    try:
        if _should_use_gemini_grounded_search(
            ...
            web_query = build_live_search_query(
                ...
                creator_niche=" ".join(
                    part for part in [creator_terms.get("niche"), creator_terms.get("identity_terms")] if part
                ),
                preferred_platforms=preferred_platforms,
...
def _build_public_fact_fallback(
    ...
    voice_cues: List[str] = []
    if sig_phrases:
        voice_cues.append(f"Use these vocabulary pattern cues without copying them: {', '.join(sig_phrases[:2])}")
    ...
    if voice_hint or sig_phrases:
        ...
        try:
            mini_prompt = (
                ...
                f"without using a generic website/profile handoff. If useful, ask for a sharper angle or say you need a verified source before giving the exact fact.\n"
                f"VOICE RULES: {voice_hint}\n"
    ...
    if is_catalog_question:
        if "book" in lowered:
            return (
                "I do not have a verified full book list in this chat yet, so I would rather keep it honest than guess the catalog."
            )
        return (
            "I do not have a verified full list in this chat yet, so I would rather keep it honest than guess the catalog."
        )
    if is_timeline_question:
        return (
            "I do not have a verified date for that in this chat yet, so I am not going to make up the timeline."
        )
    if any(token in lowered for token in ["followers", "subscribers", "members"]):
        return (
            "I do not have that exact current public detail verified in this chat yet, so I am going to keep it honest instead of guessing."
        )
    if any(
        phrase in lowered
        for phrase in [
        ...
        return (
            "I do not have that exact current public detail verified in this chat yet, so I am not going to make up a number."
        )
    return (
        "I do not have that exact public detail verified in this chat yet, so I am going to keep it honest instead of guessing."
    )
...
def generate_grounded_answer(
    ...
    vibe_prompt_block = format_vibe_prompt_block(detect_message_vibe(question, conversation_history or []))
    
    # Construct Render Prompt
    render_system_prompt = f"""
You are {creator_name or 'the creator'}.
Speak in first person. Do not refer to yourself by creator name or in third person unless the user explicitly asks who you are. Never open with "{creator_name or 'the creator'} is here" or "{creator_name or 'the creator'} is in the building".
...
VOICE PRIMACY: Every sentence must sound like the creator's first-person voice, not a generic expert or a narrator describing {creator_name or 'the creator'}. Anchor the reply to at least one creator-specific stance, phrase, rule, story, or grounded source when natural.
...
12. NO THIRD-PERSON SELF-INTRO: Do not write "{creator_name or 'the creator'} is here", "{creator_name or 'the creator'} is in the building", or any sentence that describes yourself by name. Start with the answer or a natural first-person line.
...
"""
    ...
    if creator_profile:
        ...
        if isinstance(style_fp, dict):
            ...
            creator_markers += clean_style_phrase_list(lexical_rules.get("signature_phrases") or [], limit=4)
...
def _get_suggested_resources(history: Optional[List[Dict[str, str]]]) -> Dict[str, Set[str]]:
    ...
    for m in history[-50:]:
        ...
        natural += re.findall(
            r'(?:video|podcast|episode|post|reel|clip|resource)\s+(?:called|titled|named|of)\s+([^.!?\n]{4,140})',
            content,
        )
        natural += re.findall(
            r'\bon\s+(?:an?\s+|the\s+)?(?:episode\s+of\s+)?([^.!?\n]{4,140}?(?:podcast|show|channel))\b',
            content,
        )
...
def _build_resource_search_query(
    ...
    if history:
        ...
        if _is_followup_resource_request(user_message, last_assistant):
            contextual_user_query = build_search_query(user_message, history)
            router_query = str((resource_intent or {}).get("query") or "").strip()
            if router_query and router_query.lower() not in contextual_user_query.lower():
                return f"{contextual_user_query} {router_query}".strip()
            if contextual_user_query:
...
def grounded_rag_ask(
    ...
    conversation_history = conversation_history or []
    if not _is_pure_social_greeting(question) and not _is_light_clarification_turn(question):
    ...
    crisis_intent = detect_crisis_intent(question)
    crisis_followup_intent = None if crisis_intent else detect_crisis_followup_intent(question, conversation_history)
    crisis_route = crisis_intent or crisis_followup_intent
    if crisis_route:
        logger.info("Crisis fast-path triggered in grounded_rag_ask: %s", crisis_route.to_dict())
        crisis_creator = None
        try:
            crisis_creator = _get_creator_profile_row(creator_id, ["creator_category", "style_fingerprint", "voice_profile"])
        except Exception as exc:
            logger.warning("Crisis fast-path profile lookup skipped: %s", exc)
        return {
            ...
            "sources": [],
            "cards": [],
            "meta": {"route": "crisis_fast_path", "question_type": "self_harm", **crisis_route.to_dict()},
        }
...
Message: {answer_text[:500]}"""
    ...
    if isinstance(style_fingerprint, str):
        try:
            ...
            style_fingerprint = {}
    style_fingerprint = sanitize_style_fingerprint_for_runtime(style_fingerprint)
    enabled_platforms = get_enabled_platforms_for_creator(creator_id)
    ...
    strict_rag_only = _is_strict_rag_mode(creator_row)

    image_result = None
    if images:
        logger.info("Pipeline Step 1.5: Inspecting attached images early...")
        try:
            ...
            logger.warning("Early image inspection failed, continuing without visual shortcut: %s", exc)
            image_result = None
        if image_result and image_result.get("handled"):
            return apply_final_polish({
                "answer": image_result.get("answer", "I can tell you what I see, but I wouldn't want to guess beyond the image."),
        ...
        if image_result and not _image_turn_wants_creator_sources(question):
            visual_response = image_identity_service.build_visual_turn_response(
                question=question,
                observation=image_result.get("observation") or {},
                creator_profile=creator_row,
            )
            return apply_final_polish({
                "answer": visual_response.get("answer") or "I can see the image, but I would not want to fake a read beyond what is visible.",
                "retrieved": [image_result.get("support_chunk")] if image_result.get("support_chunk") else [],
                "sources": [],
                "cards": [],
                "meta": visual_response.get("meta") or {"question_type": "visual_chat"},
            }, creator_row.get("rhythm_profile_json"), csm, mvc_score=0, plan=None)
        if image_result:
            image_redirect = image_identity_service.build_off_domain_visual_redirect(
                question=question,
                observation=image_result.get("observation") or {},
                creator_profile=creator_row,
            )
            if image_redirect:
                return apply_final_polish({
                    "answer": image_redirect.get("answer") or "I can see the image, but that topic is outside what I work on here.",
                    "retrieved": [image_result.get("support_chunk")] if image_result.get("support_chunk") else [],
                    "sources": [],
                    "cards": [],
                    "meta": image_redirect.get("meta") or {},
                }, creator_row.get("rhythm_profile_json"), csm, mvc_score=0, plan=None)
    ...
    if images:
        if image_result is None:
            ...
            logger.info("Pipeline Step 4.5: Reusing early image inspection.")
        ...
        if not _image_turn_wants_creator_sources(question):
            visual_response = image_identity_service.build_visual_turn_response(
                question=question,
                observation=image_result.get("observation") or {},
                creator_profile=creator_row,
    ...
    smart_decision = None
    turn_context_note = ""
    smart_router, smart_should_use = _smart_intent_router_available()
    if smart_router and smart_should_use and smart_should_use(
        question,
        route="ROUTE_2_TASK",
        question_type=route_q_type,
        rule_intent=rule_intent,
        history=conversation_history,
    ):
        smart_decision = smart_router.classify(
            question,
            history=conversation_history,
            timeout_seconds=getattr(settings, "SMART_INTENT_ROUTER_TIMEOUT_SECONDS", 3.5),
        )
    if smart_decision:
        if _smart_decision_is_crisis(smart_decision):
            return apply_final_polish({
                "answer": build_crisis_response(user_name=user_name, creator_profile=creator_row),
                "retrieved": [],
                "sources": [],
                "cards": [],
                "meta": {"route": "crisis_smart_intent", "question_type": "self_harm"},
            }, creator_row.get("rhythm_profile_json"), csm, mvc_score=0, plan=None)
        if _smart_decision_requires_boundary(smart_decision):
            boundary_answer = _build_smart_boundary_response(
                question=question,
            ...
            return apply_final_polish({
                ...
                "cards": [],
                "meta": {
                    "route": "smart_boundary",
                    "question_type": _smart_decision_value(smart_decision, "question_type", "boundary"),
                    "response_mode": "boundary",
                },
            }, creator_row.get("rhythm_profile_json"), csm, mvc_score=0, plan=None)
        _, route_q_type, route_topic, rule_intent, _ = _apply_smart_routing_decision(
            smart_decision,
            route="ROUTE_2_TASK",
        ...
        route_evidence_plan = _apply_smart_evidence_plan(smart_decision, route_evidence_plan, question=question)
        turn_context_note = _smart_turn_context_note(smart_decision, question)
    should_route_personal = _should_route_personal_fact_question(
        ...
        personal_question_flag=(
            bool(user_state.get("flags", {}).get("personal_question_flag"))
            or bool(_smart_decision_value(smart_decision, "is_creator_fact", False))
        ),
    )
    if _is_creator_identity_intro_question(question) and not strict_rag_only and _smart_decision_allows_identity_shortcut(smart_decision):
    ...
    if intent in ["greeting", "greeting_only", "small_talk"]:
        ...
        context_needs_resource = _is_followup_resource_request(question, last_bot_msg)
        should_run_recommender = _should_run_resource_recommender(question, conversation_history, last_bot_msg)
        content_summary_request = is_content_summary_request(question)
        if content_summary_request:
            should_run_recommender = False
        ...
        wants_link = (explicit_link_request or context_needs_resource) and not content_summary_request
        video_intent_kws = ["video", "watch", "reel", "short", "clip", "tutorial"]
        explicit_video_request = any(kw in question.lower() for kw in video_intent_kws)
        if _followup_resource_targets_non_video(question, last_bot_msg):
            explicit_video_request = False
        is_video_request = (
            explicit_video_request
            or (context_needs_resource and _followup_resource_requires_video(question, last_bot_msg))
        ) and not content_summary_request
        visual_only_image_turn = bool(images) and not _image_turn_wants_creator_sources(question)
        if visual_only_image_turn:
            should_run_recommender = False
        evidence_plan = route_evidence_plan or _build_evidence_plan(
            question,
            creator_row,
            conversation_history,
            smart_decision=smart_decision,
        ...
        pre_search_decision = (
            SearchDecision(False, "image_visual_context", "Image turn should use Gemini visual evidence first", "pre_retrieval", 1.0)
            if visual_only_image_turn
            else search_engine.pre_retrieval_decision(
                question,
                conversation_history=conversation_history,
                turn_decision=smart_decision,
            )
    ...
    if intent in ["greeting", "greeting_only", "small_talk"]:
        ...
        corpus_search_enabled = (bool(evidence_plan and evidence_plan.should_search_corpus) or content_summary_request) and not locals().get("visual_only_image_turn", False)
        support_set = (
        ...
        if resource_locked:
            ...
            voice_support_set = _build_voice_support_chunks(support_set)
        elif not support_set and corpus_search_enabled:
            ...
            transcript_candidates = (
                retrieve_candidates(creator_id, rec_result.get("q_emb"), 3, enabled_platforms=enabled_platforms)
        ...
        if not strict_rag_only:
            ...
            support_set = _filter_strict_rag_support(support_set)
        ...
        try:
            ...
            cache_lookup_needed = _should_use_gemini_context_cache_for_turn(
                question,
                support_set,
                wants_link=wants_link if "wants_link" in locals() else False,
                is_video_request=is_video_request if "is_video_request" in locals() else False,
                should_run_recommender=bool(locals().get("should_run_recommender", False)),
                content_summary_request=bool(locals().get("content_summary_request", False)),
            )
            if not strict_rag_only and cache_lookup_needed:
            ...
            elif not strict_rag_only:
                logger.info(
                    "[LATENCY] Gemini dynamic cache skipped: support_top=%.3f wants_link=%s video=%s recommender=%s summary=%s",
                    _support_set_top_score(support_set) or 0.0,
                    wants_link if "wants_link" in locals() else False,
                    is_video_request if "is_video_request" in locals() else False,
                    bool(locals().get("should_run_recommender", False)),
                    bool(locals().get("content_summary_request", False)),
                )
        except Exception as exc:
    ...
    answer = interaction_engine.render_response(
        ...
        turn_context=turn_context_note,
    )
    ...
    _should_try_cards = _should_build_source_cards_for_turn(
        smart_decision,
        answer_text=answer,
        wants_link=bool(locals().get("wants_link", False)),
        is_video_request=bool(locals().get("is_video_request", False)),
        should_run_recommender=bool(locals().get("should_run_recommender", False)),
        video_policy=getattr(plan_obj.grounding, "video_policy", ""),
        has_substantive_rag=_has_substantive_rag,
    )
    ...
    if card and not image_turn_active:
        card = _align_response_cards_with_answer(answer, support_set, card)
    ...
    should_emit_citations = _should_emit_citations_for_turn(
        smart_decision,
        answer_text=answer,
        wants_link=bool(locals().get("wants_link", False)),
        is_video_request=bool(locals().get("is_video_request", False)),
...
def _complete_stream_prefix(text: str, min_words: int = 8) -> str:
    """Return the completed sentence prefix from a cut-off stream, if useful."""
    ...
    candidate = cleaned[:last_end].strip()
...
def _looks_like_truncated_stream_answer(text: str) -> bool:
    ...
    dangling_endings = {
        ...
        "his", "her", "our", "my", "how", "what", "which", "who", "where", "when", "why",
...
async def _stream_prepared_task_response(
    *,
    creator_row: Dict[str, Any],
    creator_id: int,
    user_id: int,
    ...
    support_set: List[Dict[str, Any]],
    voice_support_set: Optional[List[Dict[str, Any]]] = None,
    rec_result: Optional[Dict[str, Any]] = None,
    preferred_platforms: Optional[List[str]] = None,
    pre_fetched_memories: Optional[List[Dict[str, Any]]] = None,
    cache_hit: bool = False,
    smart_decision: Optional[Any] = None,
):
    """Render a task answer from an already-prepared evidence packet."""
    import asyncio

    route = "ROUTE_2_TASK"
    support_set = shape_support_set(question, list(support_set or []), limit=4)
    voice_support_set = list(voice_support_set or support_set[:2])
    rec_result = rec_result or {
        "best_candidate": None,
        "confidence": 0.0,
        "resource_intent": {"preferred_platforms": preferred_platforms or []},
    }
    if preferred_platforms is None:
        preferred_platforms = ((rec_result.get("resource_intent") or {}).get("preferred_platforms") or [])
    turn_context_note = _smart_turn_context_note(smart_decision, question)

    status = "checking_memory" if cache_hit else "formulating_response"
    yield f"__STATUS__{status}"
    yield "__STATUS__formulating_response"

    render_phase_t0 = time.perf_counter()
    resource_intent = (rec_result or {}).get("resource_intent") or {}
    prepared_wants_link = needs_links(question)
    prepared_is_video_request = (
        str(resource_intent.get("resource_type") or "").lower() == "video"
        or bool(re.search(r"\b(video|videos|episode|episodes|clip|clips|watch)\b", question or "", re.IGNORECASE))
    )
    prepared_should_run_recommender = bool(
        (rec_result or {}).get("best_candidate")
        or (rec_result or {}).get("recommended")
        or (rec_result or {}).get("candidates")
    )
    titles_t0 = time.perf_counter()
    title_catalog_needed = _should_fetch_title_catalog_for_turn(
        question,
        wants_link=prepared_wants_link,
        is_video_request=prepared_is_video_request,
        should_run_recommender=prepared_should_run_recommender,
        content_summary_request=is_content_summary_request(question),
        support_set=support_set,
    )
    all_video_titles = (
        await asyncio.to_thread(fetch_all_document_titles, creator_id)
        if title_catalog_needed
        else []
    )
    logger.info(
        "[LATENCY] prepared_titles_fetch needed=%s creator_id=%s titles=%s elapsed_ms=%.1f",
        title_catalog_needed,
        creator_id,
    ...
    stream = await interaction_engine.render_combined_pass_stream_async(
        ...
        history=conversation_history or [],
        user_preferences=user_preferences,
        pre_fetched_memories=pre_fetched_memories or [],
        route=route,
        voice_chunks=voice_support_set,
    ...
    async for chunk in stream:
        ...
        if content:
            if not first_content_logged:
                ...
                logger.info(
                    "[LATENCY] stream_first_content cache_hit=%s route=%s creator_id=%s elapsed_ms=%.1f",
                    cache_hit,
                    route,
                    creator_id,
    ...
    raw_final_stream_text = "".join(streamed_parts)
    final_stream_text = raw_final_stream_text
    if (
        not raw_final_stream_text.strip()
        or _stream_stopped_for_length(finish_reasons)
        or _looks_like_truncated_stream_answer(raw_final_stream_text)
    ):
        salvage_text = _complete_stream_prefix(raw_final_stream_text)
        if salvage_text:
            logger.warning(
                "Prepared stream ended incomplete cache_hit=%s chars=%s finish_reasons=%s. Using completed prefix chars=%s.",
                cache_hit,
                len(raw_final_stream_text),
                finish_reasons,
                len(salvage_text),
            )
            final_stream_text = salvage_text
        else:
            logger.warning(
                "Prepared stream produced incomplete output cache_hit=%s chars=%s finish_reasons=%s text=%r. Falling back.",
                cache_hit,
                len(final_stream_text),
                finish_reasons,
                raw_final_stream_text[:160],
            )
            plan_obj = interaction_engine.build_interaction_plan(
                question,
                conversation_history or [],
        ...
        visible_final_stream_text = clean_response(
            raw_final_stream_text,
            strip_hyphens=should_strip_hyphens(creator_row),
        )
        if visible_final_stream_text and visible_final_stream_text != raw_final_stream_text:
            final_stream_text = visible_final_stream_text

    final_stream_text = clean_response(
        final_stream_text,
        strip_hyphens=should_strip_hyphens(creator_row),
    )
    ...
    stream_cards = []
    if _should_build_source_cards_for_turn(
        smart_decision,
        answer_text=final_stream_text,
        has_substantive_rag=_has_substantive_rag_stream,
    ):
    ...
    if stream_cards:
        stream_cards = _align_response_cards_with_answer(final_stream_text, support_set, stream_cards)
    ...
    stream_citations = (
        build_inline_citations(
            support_set,
            question=question,
            answer_text=final_stream_text or raw_final_stream_text,
            marker_provenance=getattr(interaction_engine, "get_prompt_provenance", lambda _t: [])(thread_id),
        )
        if _should_emit_citations_for_turn(
            smart_decision,
            answer_text=final_stream_text or raw_final_stream_text,
        )
        else []
...
async def grounded_rag_stream(
    ...
    conversation_history = conversation_history or []
    pure_social_greeting = _is_pure_social_greeting(question)
    if not pure_social_greeting and not _is_light_clarification_turn(question):
        resolved_question = decision_service.resolve_followup_question(question, conversation_history)
        if resolved_question != question:
            logger.info("Resolved short follow-up '%s' to '%s'", question, resolved_question)
            question = resolved_question
    ...
    crisis_route = crisis_intent or crisis_followup_intent
    if crisis_route:
        logger.info("Crisis fast-path triggered in grounded_rag_stream: %s", crisis_route.to_dict())
        yield "__STATUS__safety_check"
        crisis_creator = None
        try:
            crisis_creator = await asyncio.to_thread(
                _get_creator_profile_row,
                creator_id,
                ["creator_category", "style_fingerprint", "voice_profile"],
            )
        except Exception as exc:
            logger.warning("Crisis fast-path profile lookup skipped: %s", exc)
    ...
    route = "ROUTE_0_GREETING" if pure_social_greeting else interaction_engine.classify_route(question, conversation_history)
    if images and route in {"ROUTE_0_GREETING", "ROUTE_1_SMALL_TALK"}:
        route = "ROUTE_2_TASK"
    
    # Early heartbeat yield to drop TTFB to <100ms
    yield "__STATUS__routing"
    yield " "

    # FAST-PATH: Social Intent Bypass
    social_key = _is_social_request(question)
    if social_key:
        yield "__STATUS__checking_profiles"
        creator_row = await asyncio.to_thread(
            db.execute_one, 
            "SELECT name, handle, platform_configs, search_mode FROM creators WHERE id = %s",
            (creator_id,)
        )
        if not creator_row:
        ...
        strict_social_rag_only = _is_strict_rag_mode(creator_row)

        if not strict_social_rag_only and url:
            card_html = f"Here is the verified link you requested:\n\n[{social_key.title()}]({url})"
            yield card_html
            return

        # Fallback to research provider SOCIAL_LOOKUP. Strict RAG mode falls
        # through to the normal corpus route instead of using profile links or
        # public lookup, so links can only come from ingested content.
        if not strict_social_rag_only and social_key != "website":
            from backend.services.research_provider import GeminiResearchProvider
    ...
    if route == "ROUTE_2_TASK":
        ...
        if smart_router and smart_should_use and smart_should_use(
            ...
            smart_router_task = asyncio.create_task(
                smart_router.classify_async(
                    question,
                    history=conversation_history,
                    timeout_seconds=getattr(settings, "SMART_INTENT_ROUTER_TIMEOUT_SECONDS", 3.5),
                )
            )
    legacy_domain_checks_enabled = smart_router_task is None and route == "ROUTE_2_TASK"

    if images:
        yield "__STATUS__analyzing_image"
        creator_row = creator_row if creator_row is not None else await creator_task
        if not creator_row:
            yield "I couldn't find information about that creator."
            return
        creator_task = asyncio.create_task(asyncio.sleep(0, result=creator_row))
        try:
            image_result = await asyncio.to_thread(
                image_identity_service.inspect,
                question=question,
                images=images,
                creator_id=creator_id,
                creator_profile=creator_row,
            ...
            logger.warning("Streaming image inspection failed, continuing without visual context: %s", exc)
            image_result = None

        if image_result and image_result.get("handled"):
            yield "__STATUS__visual_reasoning"
            image_sources = image_result.get("sources") or []
            if image_sources:
                yield f"__CITATIONS__{json.dumps(image_sources)}"
            answer = image_result.get("answer") or "I can tell you what I see, but I would not want to guess beyond the image."
            answer = clean_response(answer, strip_hyphens=should_strip_hyphens(creator_row))
            yield answer
            return
        ...
        if image_result and not _image_turn_wants_creator_sources(question):
            ...
            visual_response = await asyncio.to_thread(
                image_identity_service.build_visual_turn_response,
                question=question,
                observation=image_result.get("observation") or {},
                creator_profile=creator_row,
            )
        ...
        if image_result:
            ...
            if image_redirect:
                yield "__STATUS__checking_visual_scope"
                answer = image_redirect.get("answer") or "I can see the image, but that topic is outside what I work on here."
                answer = clean_response(answer, strip_hyphens=should_strip_hyphens(creator_row))
                yield answer
                return
    ...
    if route == "ROUTE_2_TASK" and route_q_type not in {"personal_bio", "user_memory"} and rule_intent not in {"personal_bio_question", "identity", "user_memory"}:
    ...
    if legacy_domain_checks_enabled and detect_external_live_fact_topic(question):
    ...
    if legacy_domain_checks_enabled and detect_general_knowledge_topic(question):
    ...
    if legacy_domain_checks_enabled and route == "ROUTE_2_TASK":
        ...
        if creator_row:
            ...
            if _focus != "general":
                ...
                _generic_standalone_question = bool(
                    re.search(
                        r"\b(what\s+(?:is|are)|what'?s|define|explain|teach\s+me|fundamentals?|basics?|principles?|"
                        r"software\s+engineering|computer\s+science|algorithm|data\s+structure|breadth\s+first\s+search|"
                        r"depth\s+first\s+search|capital|president|who\s+invented|what\s+year|which\s+country|population)\b",
                        _q_lower,
                    )
                )
                _has_followup_reference = bool(
                    re.search(r"\b(it|its|that|this|those|these|one|same|above|previous|last|thing|mean)\b", _q_lower)
                )
                _is_obviously_on_domain = bool(
                    ...
                    or (_word_count <= 6 and not _generic_standalone_question)
                    # Conversational continuation signals
                    or bool(
                        conversation_history
                        and _word_count <= 12
                        and _has_followup_reference
                        and not _generic_standalone_question
                        and not re.search(r"\b(how to cook|recipe|weather|score|game)\b", _q_lower)
                    )
                )
    ...
    if creator_task:
        try:
            ...
            pass
    if smart_router_task:
        try:
            personal_route_candidate = bool(
                route_q_type == "personal_bio"
                or rule_intent == "personal_bio_question"
                or route_creator_personal
                or bool(route_evidence_plan and getattr(route_evidence_plan, "should_search_web", False))
                or _is_explicit_creator_timeline_query(question)
            )
            join_timeout = max(0.0, float(getattr(settings, "SMART_INTENT_ROUTER_JOIN_TIMEOUT_SECONDS", 3.5)))
            if personal_route_candidate:
                join_timeout = max(
                    join_timeout,
                    float(getattr(settings, "SMART_INTENT_ROUTER_TIMEOUT_SECONDS", 3.5)),
                )
            try:
                smart_decision = await asyncio.wait_for(
                    asyncio.shield(smart_router_task),
                    timeout=join_timeout,
                )
            except asyncio.TimeoutError:
                if personal_route_candidate:
                    logger.info("Gemini public-fact turn brain is still deciding; waiting to avoid guessing.")
                    smart_decision = await smart_router_task
                else:
                    logger.info("Gemini turn brain timed out; continuing with deterministic routers for this task.")
                    smart_router_task.cancel()
        except asyncio.TimeoutError:
            logger.info("Gemini turn brain did not return a decision.")
        except Exception as exc:
            logger.info("Gemini turn brain failed before routing: %s", exc)
    if smart_decision:
        if _smart_decision_is_crisis(smart_decision):
            creator_row = creator_row if creator_row is not None else await creator_task
            yield build_crisis_response(user_name=user_name, creator_profile=creator_row)
            return
        if _smart_decision_requires_boundary(smart_decision):
            creator_row = creator_row if creator_row is not None else await creator_task
            yield await asyncio.to_thread(
                _build_smart_boundary_response,
                question=question,
                creator_row=creator_row,
                conversation_history=conversation_history,
            )
            return
        route, route_q_type, route_topic, rule_intent, _ = _apply_smart_routing_decision(
            smart_decision,
            route=route,
            route_q_type=route_q_type,
            route_topic=route_topic,
    ...
    should_route_personal = _should_route_personal_fact_question(
        ...
        route_evidence_plan=route_evidence_plan,
        route_creator_personal=route_creator_personal,
        personal_question_flag=bool(_smart_decision_value(smart_decision, "is_creator_fact", False)),
    )
    prompt_only_task = False
    if should_route_personal and _should_prompt_only_personal_fact(smart_decision):
        logger.info(
            "Smart intent routing kept creator bio question on prompt-only stream goal=%s reason=%s",
            _smart_decision_value(smart_decision, "query_goal", ""),
            _smart_decision_value(smart_decision, "reason", ""),
        )
        should_route_personal = False
        prompt_only_task = True

    if route_q_type == "user_memory":
        yield "__STATUS__checking_memory"
        creator_row = await creator_task
        if not creator_row:
            yield "I don't have the chat context loaded right now."
            return
        try:
            if getattr(interaction_engine, "memory", None):
                mems = await interaction_engine.memory.search_async(
                    str(creator_id), str(user_id), str(thread_id or "new"), question
                )
        except Exception as exc:
            logger.warning("User-memory status path memory search failed: %s", exc)
            mems = []

        yield "__STATUS__formulating_response"
        memory_stream = await interaction_engine.render_combined_pass_stream_async(
            creator_profile=creator_row,
            rag_chunks=[],
            creator_id=creator_id,
            user_id=user_id,
        ...
        streamed_parts: List[str] = []
        async for chunk in memory_stream:
            content = _extract_stream_chunk_text(chunk)
            if content:
                streamed_parts.append(content)
        memory_text = "".join(streamed_parts)
        cleaned_memory_text = clean_response(memory_text, strip_hyphens=should_strip_hyphens(creator_row))
        if cleaned_memory_text.strip():
            yield cleaned_memory_text
        else:
            if user_name:
                yield f"Your name is {user_name}."
            else:
                yield "I do not know your name yet."
        return

    if _is_creator_identity_intro_question(question) and _smart_decision_allows_identity_shortcut(smart_decision):
        creator_row = creator_row if creator_row is not None else await creator_task
        creator_task = asyncio.create_task(asyncio.sleep(0, result=creator_row))
        if not creator_row:
            yield "I couldn't find information about that creator."
            return
        if not _is_strict_rag_mode(creator_row):
    ...
    if should_route_personal:
        ...
        if not creator_row:
            yield "I couldn't find information about that creator."
            return
        personal_allows_web = _normalize_creator_search_mode(creator_row.get("search_mode")) == "hybrid"
        if personal_allows_web and route_evidence_plan and route_evidence_plan.should_search_web and route_evidence_plan.primary_world in {"creator_world", "live_world"}:
    ...
    if prompt_only_task:
        ...
        if not creator_row:
            ...
            return
        creator_task = asyncio.create_task(asyncio.sleep(0, result=creator_row))
        if early_embedding_task is not None and hasattr(early_embedding_task, "close"):
            early_embedding_task.close()
        persona = creator_row.get("soul_md") or ""
        support_set = []
        voice_support_set = []
        mems = []
        logger.info(
            "[LATENCY] prompt_only_task creator_id=%s goal=%s intent=%s",
            creator_id,
            _smart_decision_value(smart_decision, "query_goal", ""),
            _smart_decision_value(smart_decision, "intent", ""),
        )

    if route == "ROUTE_2_TASK" and not prompt_only_task:
        creator_row = creator_row if creator_row is not None else await creator_task
        if not creator_row:
            yield "I'm sorry, I couldn't find the profile for this creator."
            return
        ...
        strict_rag_only = _is_strict_rag_mode(creator_row)
        try:
            if strict_rag_only or images:
                cached_context = None
            else:
                cached_context = await asyncio.to_thread(
                    thread_context_cache.get_reusable_context,
                    user_id=user_id,
                    creator_id=creator_id,
                    thread_id=thread_id,
                    question=question,
            ...
            logger.warning("Thread context cache lookup skipped: %s", exc)
            cached_context = None
        if cached_context:
            if early_embedding_task is not None and hasattr(early_embedding_task, "close"):
                early_embedding_task.close()
            logger.info(
                "[LATENCY] thread_context_cache_hit creator_id=%s thread_id=%s score=%s reason=%s",
                creator_id,
                thread_id,
                cached_context.get("_reuse_score"),
                cached_context.get("_reuse_reason"),
            )
            try:
                cached_mems = await asyncio.to_thread(
                ...
                logger.warning("Thread cache path long-term memory lookup skipped: %s", exc)
                cached_mems = []
            async for prepared_chunk in _stream_prepared_task_response(
                creator_row=creator_row,
                creator_id=creator_id,
                user_id=user_id,
                thread_id=thread_id,
                ...
                conversation_history=conversation_history or [],
                user_preferences=user_preferences,
                support_set=list(cached_context.get("support_set") or []),
                voice_support_set=list(cached_context.get("voice_support_set") or []),
                rec_result=cached_context.get("rec_result") if isinstance(cached_context.get("rec_result"), dict) else None,
                preferred_platforms=((cached_context.get("rec_result") or {}).get("resource_intent") or {}).get("preferred_platforms")
                if isinstance(cached_context.get("rec_result"), dict)
                else None,
                pre_fetched_memories=cached_mems,
                cache_hit=True,
                smart_decision=smart_decision,
            ):
    ...
    if prompt_only_task:
        pass
    elif route == "ROUTE_2_TASK":
        # Full RAG Route: Needs Embeddings
        # Use the early-launched embedding task instead of creating a new one
        yield "__STATUS__searching_visual_knowledge" if image_result else "__STATUS__searching_knowledge"
        ...
        yield "__STATUS__retrieving_sources"
        ...
        search_engine = SearchDecisionEngine(creator_row)
        pre_search_decision = (
            search_engine.pre_retrieval_decision(
                question,
                conversation_history=conversation_history,
                turn_decision=smart_decision,
            )
        ...
        if should_run_recommender:
            yield "__STATUS__finding_resources"
        ...
        corpus_search_enabled = bool(evidence_plan and evidence_plan.should_search_corpus) or content_summary_request
        support_set = (
            _selected_recommendation_chunks(rec_result, preferred_platforms=preferred_platforms)
            if should_run_recommender
            else []
        ...
        if image_result and image_result.get("support_chunk"):
            yield "__STATUS__visual_reasoning"
            image_support_chunk = image_result["support_chunk"]
            support_set = [image_support_chunk, *support_set]
            voice_support_set = _build_voice_support_chunks(voice_support_set, [image_support_chunk])
        ...
        try:
            ...
            if not strict_rag_only and cache_lookup_needed:
                ...
                logger.info(
                    "[LATENCY] Gemini dynamic cache skipped in stream: support_top=%.3f wants_link=%s video=%s recommender=%s summary=%s",
                    _support_set_top_score(support_set) or 0.0,
                    wants_link,
                    is_video_request,
                    bool(locals().get("should_run_recommender", False)),
        ...
        if route == "ROUTE_0_GREETING":
            ...
            yield "__STATUS__checking_memory"
            try:
                memory_obj = getattr(interaction_engine, "memory", None)
                if memory_obj and hasattr(memory_obj, "search_async"):
                    mems = await memory_obj.search_async(
                        str(creator_id), str(user_id), str(thread_id or "new"), question
                    )
            except Exception as exc:
                logger.warning("Small-talk memory lookup skipped: %s", exc)
                mems = []
        ...
        if route == "ROUTE_0_GREETING":
            ...
            try:
                ...
                logger.info(
                    "[LATENCY] stream_greeting_gemini creator_id=%s elapsed_ms=%.1f",
                    creator_id,
                    (time.perf_counter() - greeting_t0) * 1000.0,
                )
            except Exception as greeting_exc:
                logger.warning("Gemini greeting failed; using deterministic safety fallback: %s", greeting_exc)
                greeting_text = greeting_service.generate_greeting(
                    ...
                    style_fingerprint=style_fingerprint,
                    variation_seed=f"{thread_id or 'new'}:{creator_id}:{len(conversation_history or [])}:{time.time_ns()}",
                    conversation_history=conversation_history or [],
                    creator_profile=creator_row,
                    user_message=question,
                )
                logger.info(
                    "[LATENCY] stream_greeting_safety_fallback creator_id=%s elapsed_ms=%.1f",
                    creator_id,
                    (time.perf_counter() - greeting_t0) * 1000.0,
                )
        ...
        if route == "ROUTE_1_SMALL_TALK":
            yield "__STATUS__formulating_response"
            small_talk_question = (
                get_bridge_question(creator_row, creator_row.get("creator_category") or "general")
                or "What's on your mind?"
            )
            try:
                small_talk_plan = InteractionPlan(
                    route="ROUTE_1_SMALL_TALK",
                    next_question=small_talk_question,
                )
            except TypeError:
                small_talk_plan = InteractionPlan()
                small_talk_plan.route = "ROUTE_1_SMALL_TALK"
                small_talk_plan.next_question = small_talk_question
            small_talk_text = await asyncio.to_thread(
                interaction_engine._render_small_talk,
                small_talk_plan,
                creator_row,
                question,
                user_name,
                creator_row.get("soul_md") or "",
                user_preferences,
                conversation_history or [],
                thread_id or "new",
            )
            small_talk_text = strip_all_markdown(small_talk_text, creator_profile=creator_row)
            small_talk_text = clean_response(small_talk_text, strip_hyphens=should_strip_hyphens(creator_row))
            if small_talk_text:
                yield small_talk_text
            else:
                yield "What's up?"
            return

    if route == "ROUTE_2_TASK" and support_set and not images and not locals().get("strict_rag_only", False):
        try:
            await asyncio.to_thread(
                thread_context_cache.save_context,
                user_id=user_id,
                creator_id=creator_id,
                thread_id=thread_id,
                question=question,
                ...
                rec_result=rec_result if isinstance(locals().get("rec_result"), dict) else None,
                metadata={
                    "source": "grounded_rag_stream",
                    "support_count": len(support_set or []),
                    "web_results": len(locals().get("web_results") or []),
                    "search_mode": locals().get("search_mode"),
                },
            )
        except Exception as exc:
            logger.warning("Thread context cache save skipped: %s", exc)
    ...
    logger.info(
        "[LATENCY] stream_titles_fetch needed=%s route=%s creator_id=%s titles=%s elapsed_ms=%.1f",
        title_catalog_needed,
        route,
        creator_id,
        len(all_video_titles or []),
    ...
    if (
        ...
        if salvage_text:
            logger.warning(
                "Streaming renderer ended incomplete for route %s chars=%s finish_reasons=%s. Using completed prefix chars=%s.",
                route,
                len(raw_final_stream_text),
                finish_reasons,
                len(salvage_text),
    ...
    if _should_build_source_cards_for_turn(
        ...
        wants_link=bool(locals().get("wants_link", False)),
        is_video_request=bool(locals().get("is_video_request", False)),
        should_run_recommender=bool(route == "ROUTE_2_TASK" and locals().get("should_run_recommender", False)),
        has_substantive_rag=_has_substantive_rag_stream,
    ):
        stream_cards = _build_response_cards(
...
_CREATOR_VIDEO_PLATFORMS = {"youtube", "tiktok", "instagram", "facebook", "twitter", "x"}
