...
class LiveSearchRuleTests(unittest.TestCase):
    ...
    def test_build_live_search_query_uses_assistant_anchor_for_short_followup(self):
        history = [
        ...
        query = live_search_rules.build_live_search_query(
            ...
            history,
            creator_name="Matt Armstrong",
            creator_niche="cars automotive rebuilds",
        )
        self.assertIn("latest wreck we just pulled into the shop", query)
        self.assertIn('"Matt Armstrong"', query)
        self.assertIn("cars automotive rebuilds", query)

    def test_build_live_search_query_uses_prior_named_resource_for_link_followup(self):
        history = [
        ...
        query = live_search_rules.build_live_search_query(
            ...
            creator_niche="entrepreneurship business",
        )
        self.assertIn("The Kim Constable Podcast", query)
        self.assertIn('"Alex Hormozi"', query)
        self.assertIn("entrepreneurship business", query)
    ...
    def test_build_live_search_query_anchors_same_name_creator_with_category(self):
        query = live_search_rules.build_live_search_query(
            "i wanna start a business",
            creator_name="Matt Armstrong",
            creator_handle="@mattarmstrongbmx",
            creator_niche="cars automotive rebuilds",
            require_video=True,
        )
        self.assertIn('"Matt Armstrong"', query)
        self.assertIn("@mattarmstrongbmx", query)
        self.assertIn("cars automotive rebuilds", query)
        self.assertIn("video", query.lower())
