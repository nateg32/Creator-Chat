...
class PlatformTextExtractionTests(unittest.TestCase):
    ...
    def test_youtube_title_reads_nested_snippet_title(self):
        item = {
            "snippet": {
                "title": "My First Expensive Suit",
                "description": "Video description",
            },
        }

        with patch.object(apify_service, "fetch_youtube_oembed_title", return_value="") as oembed:
            title = apify_service.extract_title_from_metadata(
                item,
                "youtube",
                "https://www.youtube.com/watch?v=m-k0_pQJlfY",
            )

        self.assertEqual(title, "My First Expensive Suit")
        oembed.assert_not_called()

    def test_youtube_generic_id_uses_oembed_before_caption(self):
        item = {
            "title": "m-k0_pQJlfY",
            "description": "This description should not become the title first.",
        }

        with patch.object(apify_service, "fetch_youtube_oembed_title", return_value="Real Video Title"):
            title = apify_service.extract_title_from_metadata(
                item,
                "youtube",
                "https://www.youtube.com/watch?v=m-k0_pQJlfY",
