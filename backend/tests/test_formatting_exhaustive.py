...
class FormattingExhaustiveTests(unittest.TestCase):
    ...
    def test_a6_third_person_self_opener_removed_when_content_follows(self):
        raw = (
            "Anabolic Gabe is in the building to talk gains, business and consistency.\n\n"
            "Pretty much every guy goes through the same phase."
        )
        self.assertEqual(
            clean_response(raw),
            "Pretty much every guy goes through the same phase.",
    ...
    def test_a7_numbers_and_special_not_corrupted(self):
        cases = [
            "10x return",
            "1,600 books",
            "$30,000",
    ...
    def test_a8_cleaning_is_idempotent(self):
        inputs = [
            "Amazon",
            "Check it out on Amazon and Audible",
            "non-negotiable",
