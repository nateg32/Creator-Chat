...
def load_interaction_engine_module():
    ...
    fake_formatting.clean_for_stream_chunk = lambda text: text
    ...
    fake_chat_prompt = types.ModuleType("backend.services.chat_prompt")
    fake_chat_prompt.build_creator_style_disclosure_prompt = lambda *args, **kwargs: ""
    fake_chat_prompt.build_universal_human_engine_prompt = lambda *args, **kwargs: ""
    fake_chat_prompt.build_personality_filter_prompt = lambda *args, **kwargs: ""
    fake_chat_prompt.build_memory_association_prompt = lambda *args, **kwargs: ""
    fake_emotional_intelligence = types.ModuleType("backend.services.emotional_intelligence")
    fake_emotional_intelligence.detect_message_vibe = lambda *args, **kwargs: {"primary": "neutral"}
    fake_emotional_intelligence.format_vibe_prompt_block = lambda *args, **kwargs: ""
    fake_thread_memory_snapshot = types.ModuleType("backend.services.thread_memory_snapshot")
    fake_thread_memory_snapshot.thread_memory_snapshot_service = types.SimpleNamespace(
        build_prompt_block=lambda *args, **kwargs: "",
        update_after_turn=lambda *args, **kwargs: None,
    )
    ...
    with patch.dict(
        sys.modules,
        {
            ...
            "backend.services.chat_prompt": fake_chat_prompt,
            "backend.services.emotional_intelligence": fake_emotional_intelligence,
            "backend.services.thread_memory_snapshot": fake_thread_memory_snapshot,
...
class UserPreferenceTests(unittest.TestCase):
    def test_creator_checkin_routes_to_small_talk_not_repeated_greeting(self):
        self.assertEqual(interaction_engine.classify_route("Hello", []), "ROUTE_0_GREETING")
        self.assertEqual(interaction_engine.classify_route("heyyy danny whatsup", []), "ROUTE_0_GREETING")
        self.assertEqual(interaction_engine.classify_route("Hello Alex Hormozi what's up", []), "ROUTE_0_GREETING")
        self.assertEqual(
            interaction_engine.classify_route(
                "Whats up Alex what u been upto",
                [{"role": "assistant", "content": "What's up, Nathan?"}],
            ),
            "ROUTE_1_SMALL_TALK",
        )
        self.assertEqual(interaction_engine.classify_route("what have you been up to", []), "ROUTE_1_SMALL_TALK")
        self.assertEqual(interaction_engine.classify_route("huh?", []), "ROUTE_1_SMALL_TALK")
        self.assertEqual(
            interaction_engine.classify_route(
                "what do you mean?",
                [{"role": "assistant", "content": "What are you working on right now?"}],
            ),
            "ROUTE_1_SMALL_TALK",
        )
        self.assertEqual(
        ...
        self.assertEqual(interaction_engine.classify_route("hey Dan how do I start a business", []), "ROUTE_2_TASK")
        self.assertEqual(interaction_engine.classify_route("what's up with responses cutting off", []), "ROUTE_2_TASK")
    ...
    def test_render_greeting_rejects_command_parser_acknowledgement(self):
        plan = InteractionPlan(route="ROUTE_0_GREETING", next_question="What are you working on right now?")
        creator_profile = {
            "name": "Alex Hormozi",
            "creator_category": "business",
        ...
        with patch.object(
            interaction_engine,
            "_generate_completion_with_compat",
            return_value="Agree, Nathan. What's on your mind?",
        ):
            greeting = interaction_engine._render_greeting(
                plan,
                creator_profile,
        ...
        self.assertFalse(greeting.lower().startswith("agree"))
        self.assertIn("Nathan", greeting)
        self.assertIn("?", greeting)

    def test_render_greeting_rejects_truncated_question_stem(self):
        plan = InteractionPlan(route="ROUTE_0_GREETING", next_question="What are you working on right now?")
        creator_profile = {
            "name": "Alex Hormozi",
            "creator_category": "business",
        ...
        self.assertNotEqual(greeting, "Hey Nathan. What")
        self.assertIn("Nathan", greeting)
        self.assertTrue(greeting.endswith("?"))

    def test_render_greeting_rejects_dangling_want_fragment(self):
        plan = InteractionPlan(route="ROUTE_0_GREETING", next_question="What are you working on right now?")
        creator_profile = {
            "name": "Alex Hormozi",
            "creator_category": "business",
        ...
        self.assertNotEqual(greeting, "Yo Nathan. Want")
        self.assertNotIn("Want", greeting)
        self.assertIn("Nathan", greeting)
        self.assertTrue(greeting.endswith("?"))

    def test_render_small_talk_repairs_bland_creator_working_on_reply(self):
        plan = InteractionPlan(route="ROUTE_1_SMALL_TALK", next_question="What's on your mind?")
        creator_profile = {
            "name": "Alex Hormozi",
            "creator_category": "business",
            "soul_md": "I build and scale companies, talk about offers, leverage, and doing the work.",
            "voice_profile": {},
            "style_fingerprint": {},
        }
        replies = [
            "Getting everything",
            "Mostly tightening offers, content, and the stuff that creates leverage. What are you building?",
        ]

        with patch.object(interaction_engine, "_generate_completion_with_compat", side_effect=replies) as generate:
            result = interaction_engine._render_small_talk(
                plan,
                creator_profile,
                user_msg="what have u been working on",
                user_name="Nathan",
                history=[
                    {"role": "assistant", "content": "Yo Nathan."},
                    {"role": "user", "content": "how u goin ma man"},
                    {"role": "assistant", "content": "Doing good"},
                ],
            )

        self.assertEqual(generate.call_count, 2)
        self.assertNotIn("Getting everything", result)
        self.assertIn("offers", result.lower())

    def test_render_small_talk_repairs_bland_checkin_reply(self):
        plan = InteractionPlan(route="ROUTE_1_SMALL_TALK", next_question="What's on your mind?")
        creator_profile = {
            "name": "Alex Hormozi",
            "creator_category": "business",
            "soul_md": "I am direct, practical, and focused on building useful things.",
            "voice_profile": {},
            "style_fingerprint": {},
        }
        replies = [
            "Doing good",
            "Good, locked in and keeping the work tight. What are we working on?",
        ]
        ...
        with patch.object(interaction_engine, "_generate_completion_with_compat", side_effect=replies) as generate:
            result = interaction_engine._render_small_talk(
                plan,
                creator_profile,
                user_msg="how u goin ma man",
                user_name="Nathan",
                history=[],
            )

        self.assertEqual(generate.call_count, 2)
        self.assertNotEqual(result, "Doing good")
        self.assertIn("locked in", result.lower())
    ...
    def test_default_task_reply_budget_is_short_and_conversational(self):
        ...
        self.assertEqual(
            budget,
            {"max_words": 130, "max_sentences": 5, "max_paragraphs": 2, "max_tokens": 520, "detailed": False},
        )
    ...
    def test_explicit_detailed_request_keeps_larger_budget(self):
        ...
        self.assertEqual(
            budget,
            {"max_words": 320, "max_sentences": 12, "max_paragraphs": 6, "max_tokens": 1100, "detailed": True},
        )

    def test_deep_plan_request_allows_structure(self):
        self.assertTrue(
            interaction_engine._wants_detailed_response("how do i find all of that, give me a deep plan")
        )
        self.assertTrue(
            interaction_engine._should_allow_lists(None, "how do i find all of that, give me a deep plan")
        )
