...
class GreetingServiceTests(unittest.TestCase):
    def test_is_greeting_detects_social_openers(self):
        ...
        self.assertTrue(is_greeting("yoo"))
        self.assertTrue(is_greeting("yooo bro"))
        self.assertTrue(is_greeting("yoooo broskki"))
        self.assertTrue(is_greeting("yo ma g"))
        self.assertTrue(is_greeting("heyyyy"))
        self.assertTrue(is_greeting("hiiii"))
        self.assertTrue(is_greeting("heyyy danny whatsup"))
        self.assertTrue(is_greeting("hello Alex Hormozi what's up"))
        self.assertTrue(is_greeting("whatsup alex"))
        self.assertFalse(is_greeting("what's your best advice for starting a business"))
        self.assertFalse(is_greeting("yoo what workout should i start"))
        self.assertFalse(is_greeting("hey Dan how do I start a business"))
        self.assertFalse(is_greeting("what's up with the responses cutting off"))
        self.assertFalse(is_greeting("Whats up Alex what u been upto"))
    ...
    def test_greeting_does_not_use_raw_creator_content_hook(self):
        creator, voice_profile, style_fingerprint = _fixture()
        voice_profile = {
            **voice_profile,
            "signature_phrases": [
                "Bro needs to see this",
                "Pretty much every guy goes through the same phases",
                "If you know you know",
            ],
            "greeting_high_energy": [],
            "greeting_neutral": [],
            "greeting_short": [],
        }
        greeting = greeting_service.generate_greeting(
            ...
            creator_category="fitness",
            style_fingerprint={"speech_mechanics": {"signature_openings": []}},
            variation_seed="raw-content-hook",
            creator_profile={
                **creator,
                "name": "Anabolic Gabe",
                "creator_category": "fitness",
                "behavioral_fingerprint": {
                    "catchphrases": ["Bro needs to see this", "This is why the industry is f*cked"],
                },
            },
        )
        lowered = greeting.lower()
        self.assertNotIn("bro needs to see this", lowered)
        self.assertNotIn("pretty much every", lowered)
    ...
    def test_casual_user_message_shapes_greeting_without_raw_quotes(self):
        creator = {
            "name": "Anabolic Gabe",
            "creator_category": "fitness",
            "voice_patterns": {"interaction_style": {"energy_level": "high"}},
            "behavioral_fingerprint": {
                "catchphrases": ["Bro needs to see this", "If you know you know"],
            },
        }
        voice_profile = {
            "energy": {"bucket": "HIGH"},
            "signature_phrases": ["Bro needs to see this", "If you know you know"],
            "tone_traits": {"hype": 0.8, "supportive": 0.3, "blunt": 0.5},
        }
        greeting = greeting_service.generate_greeting(
            "Nathan",
            voice_profile,
            ...
            creator_category="fitness",
            style_fingerprint={},
            variation_seed="casual-user-tone",
            conversation_history=[],
            creator_profile=creator,
            user_message="yo ma g",
        )
        ...
        self.assertNotIn("bro needs to see this", lowered)
        self.assertNotIn("if you know you know", lowered)
        self.assertNotIn("good to have you here", lowered)
        self.assertLessEqual(greeting.count("?"), 1)

    def test_greeting_rejects_style_analysis_fragments_as_openers(self):
        creator, voice_profile, style_fingerprint = _fixture()
        style_fingerprint = {
            **style_fingerprint,
            "speech_mechanics": {
                "signature_openings": [
                    "Definitive statements ('Commitment means",
            ...
            "golden_examples": {
                "greeting": [
                    "Signature opening: Commitment means, then ask what's up.",
                    "Cut the fluff. What's on your mind right now?",
                ]
            },
        }
        ...
        greeting = greeting_service.generate_greeting(
            ...
            creator_category=creator["creator_category"],
            style_fingerprint=style_fingerprint,
            variation_seed="style-analysis-fragment",
            creator_profile=creator,
            user_message="yo",
        )

        lowered = greeting.lower()
        self.assertNotIn("definitive statements", lowered)
        self.assertNotIn("commitment means", lowered)
        self.assertNotIn("signature opening", lowered)
