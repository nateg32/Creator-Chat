 ...
 *   - loading / error
 *
 * Polls every `pollIntervalMs` (default 8s) so step counts/locks stay live.
 * Pass `creatorId = null` to disable.
 */
export function useWorkflow(creatorId, { pollIntervalMs = 8000 } = {}) {
  ...
  const fetchOnce = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await getCreatorWorkflow(creatorId);
      setState(data);
      setError(null);
    } catch (e) {
