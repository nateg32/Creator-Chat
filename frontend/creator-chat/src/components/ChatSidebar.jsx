...
export function ChatSidebar({
    ...
    const formatThreadSummary = (title) => {
        const raw = String(title || "").replace(/\s+/g, " ").trim();
        if (!raw) return "New chat";
        return raw
            .replace(/^new conversation$/i, "New chat")
            .replace(/\bWbu\b/g, "wbu")
            .replace(/\bU\b/g, "u");
    };
    ...
    return (
        <div className={`chat-sidebar ${isCollapsed ? "collapsed" : ""} ${isDeleteMode ? "delete-mode" : ""} ${isArchiveMode ? "archive-mode" : ""}`}>
            <div className="sidebar-header">
                        ...
                        {!isDeleteMode ? (
                            <>
                                <div className="sidebar-title-group">
                                    <h3>{isArchiveMode ? "Archived" : "Recents"}</h3>
                                </div>
                ...
                {!isCollapsed && (
                    <button onClick={handleToggle} className="toggle-button" aria-label="Collapse sidebar">
                                                        ...
                                                        {isRenaming ? (
                                                            ...
                                                            <>
                                                                <div className="thread-title">
                                                                    {formatThreadSummary(thread.title)}
                                                                </div>
                                                                <button
                                                                    type="button"
                                                                    className="thread-menu-trigger"
                                                                    aria-label="Chat options"
                                                                    onClick={(e) => toggleMenu(e, thread, creator.id)}
                                                                >
